...
export default function ContinuousFrontierSession({ config }: { config: FrontierConfig }) {
  ...
  return <>
    ...
    {session && <>
      ...
      {session.regulatory_exposure && <RegulatoryExposureCard exposure={session.regulatory_exposure} runtimeVerdict={session.steps.at(-1)?.shadow_decision || session.steps.at(-1)?.morrison_decision.verdict} unauthorizedExecutions={session.summary.unauthorized_executions} />}
