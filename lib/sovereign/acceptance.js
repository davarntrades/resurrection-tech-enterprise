...
async function run({ site = null, operator = null, witness = null, keep = false, rt = null, ops = null } = {}) {
  ...
  // A run under a connected profile proves the platform works. It proves
  // NOTHING about sovereign or air-gapped operation — the store is remote, the
  // policies come from a control plane and egress is permitted. Recording that
  // distinction here, rather than leaving it to whoever reads the PDF, is what
  // stops a cloud-profile record being presented as sovereign evidence.
  const offline = ["on_prem", "sovereign", "air_gapped"].includes(prof.id);
  ...
  return {
    ...
    profile: prof.id,
    deployment: {
      profile: prof.id, title: prof.title, storage: prof.storage, evidence: prof.evidence,
      policy_provider: prof.policy_provider, egress: prof.egress, immutable: profiles.immutable(),
    },
    sovereign_evidence: offline,
    site: site || null,
...
function document(a, { classification = null } = {}) {
  ...
  const d = a.deployment || { profile: a.profile, storage: "?", policy_provider: "?", egress: "?" };
  const sovereign = a.sovereign_evidence === true;

  // The subtitle is where a reader forms their impression, so EVERY caveat that
  // applies lives there — they COMPOSE rather than override. The two are
  // independent: a witnessed field trial on a connected profile is still not
  // evidence of sovereign operation, and an air-gapped self-test is still not a
  // field trial. Letting one caveat displace the other is how a record ends up
  // reading stronger than the run that produced it.
  const caveats = [];
  if (!sovereign) {
    caveats.push(`THIS RUN DOES NOT EVIDENCE SOVEREIGN OR AIR-GAPPED OPERATION — it used the ${d.profile} profile (storage ${d.storage}, policies ${d.policy_provider}, egress ${d.egress}). Re-run under the sovereign or air_gapped profile for that.`);
  }
  if (!a.field_trial) {
    caveats.push("NO SITE OR WITNESS WAS RECORDED, so this is a software self-test and must not be described as a field trial.");
  }
  const subtitle = caveats.length
    ? `Guardian OS acceptance run under the ${d.profile} profile. ${caveats.join(" ")}`
    : `Guardian OS Sovereign executed on the deployed system named below under the ${d.profile} profile, observed by the named witness.`;
  ...
  return {
    ...
    meta: [
      ...
      { label: "Deployment profile", value: `${d.profile}${sovereign ? "" : " — NOT a sovereign profile"}` },
      { label: "Storage / policies / egress", value: `${d.storage} / ${d.policy_provider} / ${d.egress}` },
      { label: "Evidences sovereign operation", value: sovereign ? "yes" : "NO" },
    ...
    blocks: [
      ...
      ...(sovereign ? [] : [{ kind: "note", reason: `this run used the ${d.profile} profile — state was ${d.storage}, policies came from the ${d.policy_provider} provider and egress was ${d.egress}. It demonstrates that the platform governs; it is NOT evidence of sovereign or air-gapped operation` }]),
      ...
      { kind: "h1", text: "Deployment under test" },
      { kind: "table", headers: ["Property", "Value"], rows: Object.entries(d).map(([k, v]) => [k.replace(/_/g, " "), v]) },
      { kind: "h1", text: "Steps" },
      ...
      { kind: "text", text: "The operator confirms this suite was executed on the system described above and that the recorded output has not been edited. The witness confirms they observed the run. Neither signature constitutes an accreditation, certification or authority to operate." + (a.sovereign_evidence ? "" : " This run was NOT performed under a sovereign or air-gapped profile and must not be cited as evidence of disconnected operation.") },
