...
const NIST = {
  ...
  controls: [
    ...
    c("AU-9", "Protection of audit information", STATUS.PARTIAL,
      "Decisions form a per-environment hash chain (prev_hash → entry_hash); verifyChain() recomputes it and reports the first altered or missing entry. Connector evidence carries a canonical per-record hash that the audit projection recomputes from the stored row, reporting any mismatch as a critical finding. The evidence module exposes no update or delete, and the engine blocks the agent from proposing evidence destruction.",
      ["lib/runtime/store.js verifyChain", "lib/runtime/connector-audit.js verifyEvidenceHash", "governance-service/operations_rules.py", "scripts/runtime/decisionseq.test.cjs", "scripts/runtime/evidence-hash-verification.test.cjs"],
      "Two limits. Connector evidence is hashed per record, not chained: an altered record is detected, but deleting a whole record is not, because there is no sequence to break — only decisions are chained. And evidence written before canonical hashing reports as UNVERIFIABLE rather than verified, because the original hash was taken over insertion-order JSON that a jsonb round trip does not preserve; such records are never reported as tampered."),
