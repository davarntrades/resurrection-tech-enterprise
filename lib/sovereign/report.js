...
// ── Value serialisation ─────────────────────────────────────────────────────
// An audit document must never contain "[object Object]". A caller who hands a
// table a structured value has given us MORE information, not less, so render
// it rather than stringifying it into noise. Every primitive below funnels
// through `readable`, so a field added upstream — a score that becomes
// {score, band}, an approval that becomes {action, reason} — degrades into
// something an assessor can read instead of into a defect.
const LABEL_KEYS = ["label", "title", "name", "action", "action_id", "id", "key"];
const DETAIL_KEYS = ["detail", "description", "reason", "note", "value", "summary"];

function readable(v, depth = 0) {
  if (v === null || v === undefined) return "";
  if (typeof v === "string") return v;
  if (typeof v === "number" || typeof v === "boolean") return String(v);
  if (Array.isArray(v)) return v.map((x) => readable(x, depth + 1)).filter(Boolean).join(" · ");
  if (typeof v !== "object") return String(v);
  if (depth > 2) return "…";                       // never recurse into deep graphs
  // A labelled record reads as "Label — detail"; anything else as key: value pairs.
  const label = LABEL_KEYS.map((k) => v[k]).find((x) => typeof x === "string" && x);
  const detail = DETAIL_KEYS.map((k) => v[k]).find((x) => x !== undefined && x !== null && x !== "");
  if (label) return detail !== undefined ? `${label} — ${readable(detail, depth + 1)}` : label;
  const pairs = Object.entries(v)
    .filter(([, x]) => x !== null && x !== undefined && x !== "")
    .map(([k, x]) => `${k.replace(/_/g, " ")}: ${readable(x, depth + 1)}`);
  return pairs.length ? pairs.join(" · ") : "";
}

/** Coerce an arbitrary value into a list item the `list` primitive understands. */
function listItem(x) {
  if (x === null || x === undefined) return null;
  if (typeof x === "string") return { label: x };
  if (typeof x !== "object") return { label: String(x) };
  const label = LABEL_KEYS.map((k) => x[k]).find((s) => typeof s === "string" && s);
  const detail = DETAIL_KEYS.map((k) => x[k]).find((s) => s !== undefined && s !== null && s !== "");
  if (label) return { label, detail: detail !== undefined ? readable(detail) : "", severity: x.severity };
  return { label: readable(x), severity: x.severity };
}
...
function kv(doc, items, { columns = 2 } = {}) {
  ...
  for (const row of rows) {
    ...
    row.forEach((it, i) => {
      ...
      const value = it.of !== undefined && it.of !== null ? `${readable(it.value)} / ${readable(it.of)}` : readable(it.value);
...
function table(doc, headers, rows) {
  ...
  const cellText = rows.map((r) => headers.map((_, i) => readable(r[i])));
  const want = headers.map((hdr, i) => Math.max(
    widthOf(hdr, FONTS.bold, SMALL),
    ...cellText.map((r) => Math.min(widthOf(r[i], FONTS.regular, SMALL), doc.contentWidth * 0.55)),
  ) + 14);
  ...
  for (const row of cellText) {
    const cells = row.map((c, i) => wrap(c, FONTS.regular, SMALL, widths[i] - 10));
...
function list(doc, items, { empty = "Nothing to show." } = {}) {
  const normalised = (items || []).map(listItem).filter((x) => x && x.label);
  if (!normalised.length) return note(doc, empty);
...
function cover(doc, { title, subtitle, meta = [], classification = null }) {
  ...
  for (const m of meta) {
    ...
    doc.draw(readable(m.value), MARGIN.left + 160, doc.y, { font: FONTS.bold, size: SMALL });
  }
  doc.newPage();
}
...
function evidencePackDocument(pack, { classification = null } = {}) {
  ...
  if (gp) {
    ...
    if (gp.scores) {
      // Each sub-score is { score, band } (lib/ops/managed.js health()), and the
      // set carries its own `overall` entry — which the KV above already shows.
      // Split the structure into real columns rather than letting it collapse.
      b.push({
        kind: "table", headers: ["Dimension", "Score", "Band"],
        rows: Object.entries(gp.scores)
          .filter(([k]) => k !== "overall")
          .map(([k, v]) => [
            k.replace(/_/g, " "),
            v && typeof v === "object" ? v.score : v,
            v && typeof v === "object" ? v.band : "",
          ]),
      });
    }
  } else {
  ...
  if (pack.executive_summary) {
    ...
    // `what_changed` and `risks_increased` are strings; `approve_next` is
    // { action, reason } records. Awaiting-approval is therefore a TABLE — an
    // assessor wants the action and the reason in separate columns, not one
    // flattened line, and flattening was what produced "[object Object]".
    for (const [key, label] of [["what_changed", "What changed"], ["risks_increased", "Risks that increased"]]) {
      const v = pack.executive_summary[key];
      b.push({ kind: "h2", text: label });
      if (Array.isArray(v) && v.length) b.push({ kind: "list", items: v });
      else if (typeof v === "string" && v) b.push({ kind: "text", text: v });
      else b.push({ kind: "note", reason: "nothing recorded for this period" });
    }
    const approve = pack.executive_summary.approve_next;
    b.push({ kind: "h2", text: "Awaiting approval" });
    if (Array.isArray(approve) && approve.length) {
      b.push({
        kind: "table", headers: ["Action", "Why it escalated"],
        rows: approve.map((x) => (typeof x === "string"
          ? [x, ""]
          : [x.action || x.action_id || x.label || "", x.reason || x.detail || "not recorded"])),
      });
    } else {
      b.push({ kind: "note", reason: "nothing is awaiting approval for this period" });
    }
  }
...
module.exports = { render, evidencePackDocument, verificationDocument, section, readable, listItem, blocks: { h1, h2, text, kv, table, list, code, note, rule, cover, footers } };
