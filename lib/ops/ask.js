...
const INTENTS = [
  ...
  {
    id: "handoffs",
    description: "Agent handoffs / coordination (\"Show handoffs\", \"chain of responsibility\", \"what's blocked between agents?\", \"handoffs for <org>\")",
    match: (p) => /hand.?off|chain of responsibility|coordinat|blocked (between|work)|who handed|baton/.test(p),
    handler: async (p) => {
      const orgs = await intelligence.list();
      const named = orgs.find((o) => o.name && p.includes(o.name.toLowerCase()));
      if (named) {
        const tl = await handoffsMod.timeline(named.org_id);
        return { kind: "handoffs", org: named.name, timeline: tl, evidenceUrl: `/admin/operations?view=handoffs&org=${named.org_id}`,
          text: tl.length ? tl.map((h) => `• ${h.from_agent} → ${h.to_agent}: ${(h.proposed_action && h.proposed_action.action_id) || "—"} [${h.status}${h.governance ? `, Ω ${h.governance.verdict}` : ""}]`).join("\n") : `No handoffs recorded for ${named.name}.` };
      }
      const sum = await handoffsMod.summary();
      const blocked = await handoffsMod.blockedWork({ limit: 10 });
      return { kind: "handoffs_summary", summary: sum, blocked_work: blocked, evidenceUrl: "/admin/operations?view=handoffs",
        text: `Handoffs: ${Object.entries(sum.by_status).filter(([, n]) => n).map(([s, n]) => `${s} ${n}`).join(", ") || "none"}.${blocked.length ? ` ${blocked.length} need attention:\n` + blocked.map((h) => `• ${h.from_agent} → ${h.to_agent}: ${(h.proposed_action && h.proposed_action.action_id) || "—"} [${h.status}]`).join("\n") : ""}` };
    },
  },
  {
    id: "pipeline",
