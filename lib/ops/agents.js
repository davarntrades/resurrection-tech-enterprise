...
// Ω rules whose repeated BLOCK is a policy-gap signal (Policy Engineering).
...
// A rule refused this many times in the window is worth a governed policy response.
const POLICY_GAP_THRESHOLD = () => Math.max(2, Number(process.env.OPS_POLICY_GAP_THRESHOLD) || 2);
...
const AGENTS = [
  ...
  {
    // ── Guardian OS departments — Incident Response ──────────────────────────
    id: "incident_response",
    title: "Incident Response Department",
    mandate: "Coordinate the response when an AI incident occurs — reconstruct the execution trajectory, identify affected agents and customers, estimate business impact, recommend containment and produce the executive incident report. Coordinates and escalates; every containment stays a governed proposal — no automatic privileged action.",
    stages: [], // cross-cutting
    actions: ["notify_operator", "raise_alert", "create_recommendation"],
    observation_kinds: ["incident.open"],
    duties(ctx, self) {
      const recs = [];
      for (const i of ctx.openIncidents || []) {
        const critical = i.severity === "critical";
        recs.push(rec(self, critical ? "raise_alert" : "notify_operator", i.org_id || null, critical ? 0.92 : 0.8,
          `Incident response: coordinate ${i.severity} incident “${i.summary || i.kind}” — reconstruct trajectory, assess impact, recommend containment`,
          critical
            ? { org_id: i.org_id || null, kind: "incident_response", severity: "critical", message: `coordinating incident ${i.id}: ${i.summary || i.kind}`, meta: { incident_id: i.id } }
            : { org_id: i.org_id || null, message: `coordinate incident ${i.id}: ${i.summary || i.kind}` },
          `incident:${i.id}`));
      }
      return recs;
    },
  ...
  {
    // ── Guardian OS departments — Runtime Risk Intelligence ──────────────────
    id: "risk_intelligence",
    title: "Runtime Risk Intelligence Department",
    mandate: "Detect trends before they become incidents. Continuously compare runtime health against the prior period — approval backlog, governed refusals, policy violations, incident rate, approval latency, customer-health drift — and surface “what changed since yesterday” with executive recommendations.",
    stages: [], // cross-cutting
    actions: ["raise_alert", "notify_operator"],
    observation_kinds: ["risk.trend"],
    duties(ctx, self) {
      const recs = [];
      const changed = (ctx.riskTrends && ctx.riskTrends.since_yesterday) || [];
      if (changed.length) {
        recs.push(rec(self, "raise_alert", null, 0.82,
          `Runtime risk: ${changed.length} adverse trend(s) vs prior period — ${changed.slice(0, 2).join("; ")}`,
          { org_id: null, kind: "runtime_risk_trend", severity: "warning", message: changed.join(" · "), meta: { trends: changed } },
          "risk.trend"));
      }
      return recs;
    },
  ...
  {
    // ── Guardian OS departments — Enterprise Architecture ────────────────────
    id: "architecture",
    title: "Enterprise Architecture Department",
    mandate: "Accelerate onboarding. Surface which customers lack a completed runtime/architecture assessment and recommend running one, so Runtime Governance insertion points and coverage gaps are found in hours, not weeks. The deep discovery of a customer's tools and trust boundaries is operator-triggered (it needs their manifest).",
    stages: [], // cross-cutting
    actions: ["create_recommendation", "notify_operator"],
    observation_kinds: [],
    duties(ctx, self) {
      const recs = [];
      for (const c of (ctx.architecture && ctx.architecture.gap_customers) || []) {
        recs.push(rec(self, "create_recommendation", c.org_id, 0.75,
          `${c.name}: no runtime/architecture assessment yet (stage ${c.lifecycle_stage}) — run an assessment to map agents, tools, trust boundaries and governance gaps`,
          { org_id: c.org_id, title: "Run enterprise architecture assessment", detail: "Discover agents, MCP servers, APIs, tools, privileged actions and trust boundaries; recommend Runtime Governance insertion points.", severity: "medium" },
          "architecture:gap"));
      }
      return recs;
    },
  ...
  {
    // ── Guardian OS departments — Policy Engineering ─────────────────────────
    id: "policy_engineering",
    title: "Policy Engineering Department",
    mandate: "Turn recurring governance signals into policy. When a rule is refused repeatedly, DRAFT a governance policy (an inert artifact) to tighten or clarify it. Everything becomes a governed proposal; drafting changes nothing — no policy becomes active without operator approval, and the agent never activates policy.",
    stages: [], // cross-cutting
    actions: ["draft_policy", "create_recommendation", "notify_operator"],
    observation_kinds: [],
    duties(ctx, self) {
      const recs = [];
      for (const g of ctx.policyGaps || []) {
        recs.push(rec(self, "draft_policy", null, 0.72,
          `Policy engineering: ${g.rule} refused ${g.count}× this week — draft a governance policy to tighten/clarify it (inert until you approve activation)`,
          { kind: "omega_rule", title: `Reinforce ${g.rule}`, target_domain: "enterprise", rationale: `${g.count} governed refusals of ${g.rule} in the last 7 days indicate a recurring pattern worth an explicit policy.`, created_by: "policy_engineering" },
          "policy:gap"));
      }
      return recs;
    },
  ...
  {
    // ── Guardian OS departments — Partner / MSSP ─────────────────────────────
    id: "partner",
    title: "Partner / MSSP Department",
    mandate: "Manage security partners and managed-service providers. Track partner organisations, deployments, renewals and white-labelled installations, and surface which partner needs attention — health decline or renewals due — as governed recommendations for the operator.",
    stages: [], // cross-cutting
    actions: ["notify_operator", "create_recommendation"],
    observation_kinds: [],
    duties(ctx, self) {
      const recs = [];
      for (const p of ctx.partners || []) {
        // Partner-level work item (no org_id) → notify_operator, not a
        // per-customer recommendation (which is org-scoped by contract).
        recs.push(rec(self, "notify_operator", null, 0.75,
          `Partner ${p.name} needs attention — ${p.reason} (${p.deployments} deployment(s))`,
          { org_id: null, message: `Partner ${p.name} (${p.kind}): ${p.reason}` },
          "partner:attention"));
      }
      return recs;
    },
...
async function context() {
  ...
  // ── Guardian OS enterprise signals for the new departments (derived) ──────
  const weekAgo = new Date(Date.now() - 7 * DAY).toISOString();
  const [openIncidents, riskTrends, arch, partnerAttention, refusals] = await Promise.all([
    incidents.list({ status: "open", limit: 200 }).catch(() => []),
    risk.trends({ windowDays: 1 }).catch(() => ({ since_yesterday: [], metrics: [], health_deltas: [] })),
    architecture.coverage().catch(() => ({ gaps: 0, gap_customers: [], coverage_pct: 100 })),
    partners.needingAttention().catch(() => []),
    opsEvidence.search({ verdict: "block", since: weekAgo, limit: 300 }).catch(() => []),
  ]);
  // Policy-gap scan: a rule refused repeatedly is a candidate for a governed
  // policy response (tighten / add an approval chain).
  const ruleCounts = {};
  for (const e of refusals) if (e.rule && POLICY_GAP_RULES.has(e.rule)) ruleCounts[e.rule] = (ruleCounts[e.rule] || 0) + 1;
  const policyGaps = Object.entries(ruleCounts).filter(([, n]) => n >= POLICY_GAP_THRESHOLD())
    .map(([rule, count]) => ({ rule, count }));

  return { orgs, orgStates, snapshot, openIncidents, riskTrends, architecture: arch, partners: partnerAttention, policyGaps };
}
