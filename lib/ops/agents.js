...
async function dispatch({ trigger = "manual" } = {}) {
  const run = await store.insert("ops_runs", {
    ...
    coordination: COORDINATION(), finished_at: null, error: null,
  });
  ...
  try {
    ...
    const coordinating = COORDINATION();
    ...
    for (const agent of AGENTS) {
      ...
      // ROUTE: emit typed handoff records for this agent's work (always).
      ...
      if (coordinating) {
        ...
        // 4.0 direct path (unchanged execution semantics; handoffs are records).
        for (const r of recs) {
    ...
    rt.log.info("ops_council", { run_id: run.id, trigger, coordinating, proposals: produced.length, handoffs_created: handoffsCreated, ...outcomes });
    return { run_id: run.id, mode: "council", trigger, coordinating, observations: ctx.snapshot.observations.length, recommendations: recCount, per_agent, outcomes, handoffs: handoffCounts, proposals: produced };
  } catch (e) {
