...
async function get(id) { return shape(await store.findOne("provisioning", { id })); }
async function list({ limit = 50 } = {}) {
  const rows = await store.find("provisioning", {});
  rows.sort((a, b) => String(b.created_at).localeCompare(String(a.created_at)));
  return rows.slice(0, limit).map(shape);
}
async function forOrg(org_id) { return (await store.find("provisioning", { org_id })).map(shape)[0] || null; }
...
/**
 * Install a governed enterprise runtime. Never throws — a failed phase marks the
 * run failed with the reason, leaving what was created intact for inspection.
 */
