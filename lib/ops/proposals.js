...
/**
 * Governance provenance for the evidence record.
 *
 * A record that says "blocked" proves a decision happened; it does not prove
 * WHICH policy produced it. Binding the ruleset hash and engine commit means a
 * later auditor can tell a verdict issued under one ruleset from the same
 * verdict issued under another — and can detect a ruleset swap between two
 * otherwise identical records.
 */
function engineProvenance(decision) {
  const att = (decision && decision.engine && decision.engine.attestation) || {};
  return {
    ruleset_hash: att.ruleset_hash || null,
    engine_commit: att.engine_commit || null,
  };
}

/**
 * Provider/action metadata worth binding into evidence.
 *
 * Deliberately narrow: identifiers and hashes only. Prompts, message bodies,
 * recipients and model output are NEVER copied into evidence — the params may
 * carry regulated content, and evidence is the one record that outlives the
 * request.
 */
function providerMetadata(params = {}) {
  const pick = {
    connector_id: params.connector_id || null,
    model_id: params.model_id || null,
    request_hash: params.request_hash || null,
    message_hash: params.message_hash || null,
    channel: (params.flags && params.flags.channel) || null,
  };
  return Object.values(pick).some((v) => v !== null) ? pick : null;
}
...
async function propose({ action_id, params = {}, org_id = null, environment_id = null, source = "operations_agent", reasoning = null, agent_id = null, hold = false }) {
  ...
  const ev = await evidence.record({
    ...
    ...engineProvenance(decision),
    provider: providerMetadata(params),
  });
...
async function approve(id, { actor, note = null }) {
  ...
  const ev = await evidence.record({
    ...
    ...engineProvenance(decision),
    provider: providerMetadata(p.params),
  });
