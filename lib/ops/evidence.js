...
async function record({
  ...
  actor = "operations_agent", agent = "resurrection-tech-ops-agent",
  ...
  const row = await store.insert("ops_evidence", {
    actor, agent, action_id, proposal_id, org_id, environment_id,
...
/** Search evidence. Filters: org_id, verdict, action_id, since (ISO). */
async function search({ org_id, verdict, action_id, since, limit = 100 } = {}) {
  const where = {};
