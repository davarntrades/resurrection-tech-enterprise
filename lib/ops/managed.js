...
async function deriveRecommendations(org_id) {
  ...
  // Industry Intelligence Packs (Phase 5) contribute domain-specific candidates.
  // They are ordinary recommendations from here on: proposed, governed, and only
  // acted on by an operator. Best-effort — a pack can never break monitoring.
  try {
    for (const r of await require("./industry").recommendations(org_id)) {
      rec(r.title, r.detail, r.severity || "warning", { pack: r.source_pack });
    }
  } catch (e) { rt.log.warn("industry_recommendations_skipped", { org_id, error: e.message }); }
