...
/**
 * Perspectives available to ONE enterprise: the eight executive roles plus a
 * lens for every installed Intelligence Pack — Industry (Phase 5) or Sovereign
 * (Phase 7). A pack lens is still the same twin; it just frames it with domain
 * intelligence.
 *
 * A National Security or Defence Operations lens sits in this list beside the
 * CEO and CISO lenses because it IS one of them: the same context, projected
 * for a mission authority instead of a board. There is no second workspace
 * system for sovereign users.
 */
async function rolesFor(org_id) {
  ...
  try {
    ...
    const packs = await industry.installed(org_id);
    for (const row of packs) {
      // resolve() covers packs installed from signed media as well as shipped
      // ones, so an air-gapped estate gets its lenses too.
      const p = await industry.resolve(org_id, row.pack_id);
      if (!p) continue;
      const sov = p.sovereign && p.sovereign.classification ? p.sovereign : null;
      base.push({
        id: `industry:${p.id}`, title: p.industry, label: p.title, purpose: p.purpose,
        industry_pack: p.id, version: p.version,
        sovereign: !!sov, classification: sov ? sov.classification : null, mission_domain: sov ? sov.mission_domain : null,
      });
    }
...
async function workspace(role, org_id) {
  // Intelligence Pack lens — the SAME shared context, projected by the pack.
  // Industry and Sovereign packs take the identical path: one lens mechanism.
  if (String(role || "").startsWith("industry:")) {
    const pack_id = String(role).slice("industry:".length);
    const industry = require("./industry");
    const p = require("./packs").get(pack_id) || (org_id ? await industry.resolve(org_id, pack_id) : null);
    if (!p) return null;
    ...
    const view = await industry.dashboard(org_id, pack_id, { ctx: ictx });
    const sov = p.sovereign && p.sovereign.classification ? p.sovereign : null;
    return {
      role, title: p.industry, label: p.title, purpose: p.purpose, industry_pack: p.id, version: p.version,
      sovereign: sov ? { classification: sov.classification, mission_domain: sov.mission_domain, mission: sov.mission, admissibility: view ? view.sovereign : null } : null,
