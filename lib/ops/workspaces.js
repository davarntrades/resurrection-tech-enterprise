...
// ── Section builders (the presentation vocabulary every workspace shares) ────
const stat = (key, title, items) => ({ key, title, kind: "stat", items: items.filter(Boolean) });
const scoreSec = (key, title, health, keys) => ({ key, title, kind: "score", overall: health ? { score: health.overall, band: health.band } : null, subs: health ? keys.map((k) => ({ key: k, label: LABEL[k] || k, ...health.scores[k] })).filter((s) => s.score != null) : [] });
const listSec = (key, title, items, empty) => ({ key, title, kind: "list", items, empty: empty || "Nothing to show." });
const timelineSec = (key, title, items, empty) => ({ key, title, kind: "timeline", items, empty: empty || "No history yet." });
...
const sev = (s) => (s === "critical" ? "critical" : s === "warning" || s === "warn" ? "warning" : "info");
...
module.exports = { ROLES, roles, context, workspace };
