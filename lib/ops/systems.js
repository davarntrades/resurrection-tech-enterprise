...
async function statusBoard() {
  const [eng, probes, m, lastRun, clientList, gmailStatus] = await Promise.all([
    rt.engine.health(),
    integrations.probeAll().catch(() => ({ integrations: [] })),
    mode(),
    agent.lastRun().catch(() => null),
    clientsMod.list().catch(() => []),
    gmailMod.status().catch(() => ({ configured: false, connected: false })),
  ]);
  ...
  // Email — read-only Gmail inbox monitoring. Configured (OAuth app creds) vs
  // connected (operator authorised a mailbox) are distinct honest states.
  cards.push(card("email",
    gmailStatus.connected ? "healthy" : gmailStatus.configured ? "awaiting_credentials" : "not_configured",
    gmailStatus.connected
      ? `read-only inbox monitoring · ${gmailStatus.mailbox}${gmailStatus.last_poll_at ? ` · last poll ${gmailStatus.last_poll_at}` : " · not yet polled"}`
      : gmailStatus.configured
        ? "Gmail OAuth configured — connect a mailbox from the Systems tab to begin read-only monitoring"
        : "Email activity unavailable — Gmail integration not configured. Manual engagement notes contribute to the briefing.",
    gmailStatus.connected
      ? { read_only: true, scope: gmailStatus.scope, store_bodies: gmailStatus.store_bodies, connect_url: null }
      : gmailStatus.configured
        ? { read_only: true, connect_url: "/api/ops/gmail/auth" }
        : { required_env: ["OPS_GMAIL_CLIENT_ID", "OPS_GMAIL_CLIENT_SECRET"] }));
