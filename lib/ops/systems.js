...
async function statusBoard() {
  ...
  // Vercel — token-gated deployments API, else site health probe.
  const vc = probe("vercel");
  const vercelConfigured = !!(process.env.OPS_VERCEL_TOKEN && process.env.OPS_VERCEL_PROJECT) || !!process.env.NEXT_PUBLIC_SITE_URL;
  cards.push(card("vercel",
    vercelConfigured ? probeStatus(vc.status) : "not_configured",
    vercelConfigured ? (vc.latest ? `latest deployment ${vc.latest.state}` : vc.probe ? `site probe ${vc.status}` : vc.error || vc.status) : "Vercel monitoring not configured",
    vercelConfigured ? {} : { required_env: ["OPS_VERCEL_TOKEN", "OPS_VERCEL_PROJECT"] }));
