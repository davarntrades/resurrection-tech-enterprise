...
/* Executor for governed mailbox READS. Authorizes only; the provider is
 * reached afterwards by the gateway, so message content never enters a
 * proposal, an execution result or an evidence row. */
async function authorizeCommunicationRead(action_id, p = {}) {
  const adapters = require("../runtime/communication-adapters");
  const action = p.canonical_action || {};
  if (action.action_id !== action_id) throw new Error("canonical action type does not match the registered communication action");
  if (!p.org_id || !p.environment_id) throw new Error("organisation and environment are required");
  if (!p.connector_id) throw new Error("connector is required");
  if (!p.request_hash) throw new Error("request hash is required");
  const spec = adapters.operationFor(action_id);
  if (!spec.reads) throw new Error("registered action is not a read action");
  const row = await rt.store.findOne("integration_connectors", { id: p.connector_id });
  if (!row || row.org_id !== p.org_id || row.environment_id !== p.environment_id) throw new Error("connector not found for this organisation and environment");
  if (row.type !== spec.adapter.connector_type) throw new Error(`connector is not a ${spec.adapter.name} connector`);
  if (row.status === "disabled" || row.health !== "healthy") throw new Error("connector must be enabled and healthy before a mailbox is read");
  return {
    authorized: true,
    action_type: action_id,
    org_id: p.org_id,
...
async function verifyCommunicationRead(action_id, result, params = {}) {
  return {
    ok: !!(result && result.authorized
      && result.action_type === action_id
      && result.reads === true
      && result.delivers === false
      && result.org_id === params.org_id
      && result.environment_id === params.environment_id
      && result.connector_id === params.connector_id
      && result.request_hash === params.request_hash),
    detail: `${action_id} authorized by Runtime Governance for the proposed organisation, environment and connector`,
  };
}
...
/* Scope-bound authorization for Salesforce and ServiceNow record operations.
 * Like communication executors, this never contacts the provider. */
async function authorizeEnterpriseAction(action_id, p = {}) {
  const adapters = require("../runtime/enterprise-action-adapters");
  const action = p.canonical_action || {};
  if (action.action_id !== action_id) throw new Error("canonical action type does not match the registered enterprise action");
  if (!p.org_id || !p.environment_id || !p.connector_id || !p.enterprise_action_run_id || !p.payload_hash) {
    throw new Error("organisation, environment, connector, enterprise run and payload hash are required");
  }
  const spec = adapters.operationFor(action_id);
  const row = await rt.store.findOne("integration_connectors", { id: p.connector_id });
  if (!row || row.org_id !== p.org_id || row.environment_id !== p.environment_id) throw new Error("connector not found for this organisation and environment");
  if (row.type !== spec.adapter.connector_type) throw new Error(`connector is not a ${spec.adapter.name} connector`);
  if (row.status === "disabled" || row.health !== "healthy") throw new Error("connector must be enabled and healthy before an enterprise action is authorized");
  return {
    authorized: true, action_type: action_id, enterprise_action_run_id: p.enterprise_action_run_id,
    org_id: p.org_id, environment_id: p.environment_id, connector_id: row.id,
    provider: spec.adapter.provider, operation: spec.operation,
    reads: !!spec.reads, mutates: !!spec.mutates, payload_hash: p.payload_hash,
  };
}
async function verifyEnterpriseAction(action_id, result, params = {}) {
  return {
    ok: !!(result && result.authorized && result.action_type === action_id
      && result.org_id === params.org_id && result.environment_id === params.environment_id
      && result.connector_id === params.connector_id && result.payload_hash === params.payload_hash),
    detail: `${action_id} authorized for the exact organisation, environment, connector and payload`,
  };
}
...
const CATALOG = {
  ...
  check_integration_connector: {
    ...
    approval_flags: {},
    execute: async (p) => {
      try { return await rt.integrationGateway.checkConnectorHealthRaw(p); }
      catch (e) {
  ...
  // Credential lifecycle for the Gmail connector. Same shape as
  // rotate_aws_bedrock_credentials: the plaintext arrives as a short-lived
  // staged secret reference, is validated live against Google, and is sealed —
  // it never enters a proposal, an execution result or an evidence row.
  rotate_gmail_credentials: {
    title: "Rotate encrypted Gmail connector OAuth credentials",
    tool: "rotate_gmail_credentials",
    risk: "medium",
    approval_flags: {},
    execute: async (p) => {
      try { return await require("../runtime/integration-gateway").rotateGmailCredentialsRaw(p); }
      catch (error) { throw new Error(`Gmail credential rotation failed: ${error.message}`); }
    },
    verify: async (result, p) => {
      const row = result && result.id ? await rt.store.findOne("integration_connectors", { id: result.id }).catch(() => null) : null;
      return {
        ok: !!(row && row.org_id === p.org_id && row.environment_id === p.environment_id && row.type === "gmail" && row.health === "healthy"),
        detail: row ? `Gmail connector ${row.id} credentials validated and rotated` : "Gmail connector missing",
      };
    },
  },
  revoke_gmail_credentials: {
    title: "Revoke Gmail connector credentials and disable the connector",
    tool: "revoke_integration_credential",
    risk: "medium",
    approval_flags: {},
    execute: async (p) => require("../runtime/integration-gateway").revokeGmailCredentialsRaw(p),
    verify: async (result) => ({
      ok: !!(result && result.status === "disabled" && result.has_secret === false),
      detail: result ? `Gmail connector ${result.id} revoked and disabled` : "Gmail connector missing",
    }),
  },
  ...
  // Governed mailbox READS. Posture matches invoke_aws_bedrock_model exactly:
  // medium risk, auto-executable after an engine PERMIT, executor authorizes
  // only, provider reached afterwards by the gateway. Reading is idempotent and
  // delivers nothing, so it carries no outbound-delivery Ω rule — but it is
  // still proposed, evaluated, permitted and evidenced before Google is
  // contacted, and a block or an unreachable engine still stops it dead.
  "gmail.list_messages": {
    title: "List mailbox messages through the governed Gmail connector",
    tool: "read_gmail_messages",
    risk: "medium",
    approval_flags: {},
    execute: async (p) => authorizeCommunicationRead("gmail.list_messages", p),
    verify: async (result, params = {}) => verifyCommunicationRead("gmail.list_messages", result, params),
  },
  "gmail.read_message": {
    title: "Read one mailbox message through the governed Gmail connector",
    tool: "read_gmail_message",
    risk: "medium",
    approval_flags: {},
    execute: async (p) => authorizeCommunicationRead("gmail.read_message", p),
    verify: async (result, params = {}) => verifyCommunicationRead("gmail.read_message", result, params),
  },
...
CATALOG.rotate_enterprise_connector_credentials = {
  title: "Rotate Salesforce or ServiceNow OAuth credentials",
  tool: "rotate_integration_credential",
  risk: "medium",
  approval_flags: {},
  execute: async (p) => rt.integrationGateway.rotateEnterpriseCredentialsRaw(p),
  verify: async (result, p) => ({
    ok: !!(result && result.id === p.connector_id && result.health === "healthy" && result.has_secret),
    detail: result ? `${result.type} connector ${result.id} credentials validated and rotated` : "connector missing",
  }),
};

/* Existing Ω vocabulary is deliberately reused:
 * - CRM/ITSM reads are medium-risk governed reads.
 * - Every external record mutation rides `modify_customer`, whose
 *   ops_unauthorized_customer_modification rule requires both a policy permit
 *   and operator approval. No new allow rule or bypass is introduced. */
for (const action of require("../runtime/enterprise-action-adapters").listActions()) {
  CATALOG[action.action_id] = {
    title: `${action.reads ? "Read from" : "Modify"} ${action.provider} through the governed Integration Gateway`,
    tool: action.mutates ? "modify_customer" : `read_${action.provider}_record`,
    risk: action.mutates ? "high" : "medium",
    approval_flags: action.mutates ? { change_authorized: true, operator_approved: true } : {},
    execute: async (p, action_id = action.action_id) => authorizeEnterpriseAction(action_id, p),
    verify: async (result, params = {}, action_id = action.action_id) => verifyEnterpriseAction(action_id, result, params),
  };
}
