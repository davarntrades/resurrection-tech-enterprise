...
const autonomyMod = require("./autonomy");
...
async function briefing() {
  ...
  // Executive Command (Phase 4): the current autonomy posture — mode, what it
  // permits on the autonomous path, and any paused specialists. Surfaces the
  // brake state at the top of the operator's briefing.
  const autonomySt = await autonomyMod.current().catch(() => null);
  const executive = autonomySt ? {
    mode: autonomySt.mode, label: autonomySt.label,
    halted: !!autonomySt.policy.halted, proposes: autonomySt.policy.proposes,
    auto_executes: autonomySt.policy.autoExecutes, holds: !!autonomySt.policy.holds,
    paused_agents: autonomySt.paused_agents, default: !!autonomySt.default,
    updated_by: autonomySt.updated_by, updated_at: autonomySt.updated_at,
  } : null;

  // Expanded Agent Council (Phase 5): security posture — governed refusals of
  // security-sensitive actions in the last 7 days. This surfaces what Runtime
  // Governance already REFUSED (evidence-destruction / credential-exposure /
  // internal-reach attempts), which the Security & Threat Agent acts on.
  const SECURITY_RULES = ["ops_evidence_destruction", "ops_credential_sharing", "ops_unauthorized_autonomy_change", "ops_internal_action_external_reach"];
  let security = null;
  try {
    const weekAgo = new Date(Date.now() - 7 * 86400000).toISOString();
    const blocked = await evidence.search({ verdict: "block", since: weekAgo, limit: 200 });
    const refusals = blocked.filter((e) => e.rule && SECURITY_RULES.includes(e.rule));
    const by_rule = {};
    for (const r of refusals) by_rule[r.rule] = (by_rule[r.rule] || 0) + 1;
    security = {
      governed_refusals: refusals.length, by_rule,
      recent: refusals.slice(0, 5).map((e) => ({ action_id: e.action_id, rule: e.rule, agent_id: e.agent_id, org_id: e.org_id, at: e.created_at })),
    };
  } catch { security = null; }
  ...
  // Provenance (Pillar 7): label every statement by class and flag any
  // observed_fact that lacks a supporting record — nothing enters the briefing
  // "as fact" without evidence.
  for (const it of items) it.provenance = graphMod.provenanceForSourceType(it.sourceType);
  const unsupported_facts = items.filter((it) => it.provenance === "observed_fact" && it.available !== false && (!it.sourceIds || it.sourceIds.length === 0) && (it.count == null || it.count > 0)).map((it) => it.id);
