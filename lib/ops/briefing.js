...
async function briefing() {
  ...
  items.push(item("followups.email", {
    message: fu.email.reason,
    severity: "info", sourceType: "email", available: false, reason: fu.email.note,
  ...
  const g = greeting();
  const text = [g.text, "", ...lines, "", recommended_actions.length ? "Recommended actions:" : "No immediate actions require your attention.",
    ...recommended_actions.slice(0, 6).map((r) => `${r.priority}. ${r.title}`)].join("\n").trim();
