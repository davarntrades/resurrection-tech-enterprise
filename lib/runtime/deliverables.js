...
const KINDS = {
  ...
  "executive-report.html": "Executive summary (HTML)",
  "executive-report.md": "Executive summary (Markdown)",
  "executive-report.pdf": "Executive summary (PDF)",
  "monthly-evidence.html": "Monthly evidence (HTML)",
  "monthly-evidence.md": "Monthly evidence (Markdown)",
  "monthly-evidence.pdf": "Monthly evidence (PDF)",
  "full-audit.html": "48-Hour Audit (HTML)",
  "full-audit.pdf": "48-Hour Audit (PDF)",
  "full-audit-model.json": "48-Hour Audit model (JSON)",
  "enterprise-assessment.html": "Enterprise Assessment (HTML)",
  "enterprise-assessment.pdf": "Enterprise Assessment (PDF)",
  "enterprise-assessment-model.json": "Enterprise Assessment model (JSON)",
  "run-summary.json": "Run summary (JSON)",
  "morrison-audit-v2.json": "Runtime evidence (verifiable JSON)",
};
const MIME = {
...
// Plan an HTTP byte response for previewing/downloading a deliverable — pure,
// so it is unit-testable without a Next runtime or object storage. Returns the
// status, headers and the [start,end] byte window to slice from the full body.
//
// Why this exists: iPad/iOS Safari's inline PDF viewer sends a `Range` request
// and refuses to render a `200` that lacks `Content-Length` / `Accept-Ranges`
// (it shows "This page couldn't load"). We answer ranges with `206` + a proper
// `Content-Range`, always advertise `Accept-Ranges: bytes`, and always send an
// accurate `Content-Length`.
function planByteResponse({ size, mime, filename, mode, range }) {
  ...
  if (m && !(m[1] === "" && m[2] === "")) {
    let start;
    let end;
    if (m[1] === "") {                        // suffix form: bytes=-N (last N bytes)
    ...
    if (Number.isNaN(start) || Number.isNaN(end) || start > end || start >= total || start < 0) {
      return { status: 416, headers: { ...base, "content-range": `bytes */${total}` }, start: 0, end: -1 };
    }
    return {
      status: 206,
  ...
  return { status: 200, headers: { ...base, "content-length": String(total) }, start: 0, end: total - 1 };
}
...
// Share passwords are a SECOND factor on top of a 144-bit token; the token is
// the control that makes a link unguessable. The password exists for the case
// the token leaks — a forwarded email, browser history, a proxy log.
//
// That is exactly why it is stored with a per-share salt and a work factor
// rather than a bare digest: an unsalted SHA-256 of a human-chosen password is
// recoverable from a leaked database by rainbow table in seconds, and customers
// reuse passwords. scrypt makes each guess cost real work, and the salt makes
// precomputation useless.
//
// Verification accepts BOTH formats. Existing shares were stored as an unsalted
// digest and must keep working — silently invalidating live customer links to
// tidy a hash format would be a worse outcome than the weakness it fixes.
...
/** Constant-time where it can be; never throws on a malformed stored value. */
function verifySharePassword(password, stored) {
  ...
  try {
    if (String(stored).startsWith(SCRYPT_PREFIX)) {
      ...
      const actual = crypto.scryptSync(supplied, Buffer.from(saltB64, "base64url"), expected.length);
      return expected.length === actual.length && crypto.timingSafeEqual(expected, actual);
    }
    // Legacy unsalted digest — still compared in constant time.
    const expected = Buffer.from(String(stored), "utf8");
    const actual = Buffer.from(store.sha256(supplied), "utf8");
    return expected.length === actual.length && crypto.timingSafeEqual(expected, actual);
  } catch { return false; }
}
...
async function createShare({ deliverable_id, expires_in_days = 7, password = null }) {
  ...
  await store.insert("shares", {
    ...
    expires_at, password_hash: password ? hashSharePassword(password) : null, revoked: false, accessed: 0,
  });
  return { token, expires_at, path: `/api/runtime/share/${token}`, filename: del.filename, org_id: del.org_id };
}

// Metadata for an existing share (for delivery — e.g. emailing the link). Never
// exposes bytes; the link stays credential-free at /api/runtime/share/<token>.
async function getShareMeta(token) {
  const s = await store.findOne("shares", { token });
  if (!s) return null;
  return { token, filename: s.filename, org_id: s.org_id, expires_at: s.expires_at, path: `/api/runtime/share/${token}`, state: shareState(s) };
}
...
async function resolveShare(token, password) {
  ...
  if (!verifySharePassword(password, share.password_hash)) return { ok: false, status: 401, error: "password required" };
...
module.exports = { KINDS, MIME, mimeFor, hashSharePassword, verifySharePassword, publishPack, publishUploaded, listPacks, getDeliverable, readBytes, planByteResponse, shareInline, createShare, getShareMeta, resolveShare, revokeShare, listShares, shareState };
