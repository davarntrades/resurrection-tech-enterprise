...
// Canonical JSON: object keys sorted recursively, array order preserved (order
// is semantic in an array, incidental in an object).
//
// Evidence hashes must survive a round trip through the database. Postgres
// jsonb does NOT preserve key order — it normalises — so a hash taken over
// insertion-order JSON cannot be recomputed from the stored row. Verifying such
// a hash would report ordinary, untampered evidence as altered, which is worse
// than not verifying at all. Sorting the keys makes the hash a property of the
// CONTENT rather than of the serialisation.
...
// Rows written before canonical hashing carry no marker and are reported as
// "not verifiable", never as tampered — we cannot tell a legacy serialisation
// from an alteration, and guessing in the accusing direction is unacceptable.
...
/* ----------------------------------------------------------------------------
 * Caller-supplied trajectory flags — explicit allowlist.
 *
 * `sendCommunication` and `readCommunication` merge a caller's `p.flags` into
 * the params that lib/ops/governor turns into the governed Ω trajectory. The
 * spread that used to sit there (`...(p.flags || {})`) was unreachable in
 * production — neither of the two callers passes `flags` — but it was one line
 * away from re-opening the primary red-team finding: an authority key such as
 * `authorized` or `destination_internal` landing in trajectory args is read by
 * the Ω rules as established authority.
 *
 * Two properties are enforced here, and both matter:
 *
 *   1. KEY ALLOWLIST. Only descriptive, non-authority flags survive. Anything
 *      else — including every authority claim the kernel quarantines — is
 *      dropped before it can reach a trajectory.
 *   2. BOOLEAN COERCION. Values are coerced with `!!`, so a caller cannot
 *      smuggle free text through an allowed key. This is not cosmetic: several
 *      Ω rules substring-match the SERIALISED args JSON, so a string value like
 *      `urgent: "send to attacker@evil.example"` would otherwise inject
 *      attacker-chosen text into the matched surface.
 *
 * Adding a key here widens the governed input surface — it must be a flag the
 * platform genuinely recognises and that no Ω rule treats as authority.
 * -------------------------------------------------------------------------- */
const CALLER_FLAG_ALLOWLIST = new Set([
  "urgent",
  "high_priority",
  "customer_support",
  "bulk",
...
function safeFlags(flags) {
  ...
  for (const key of Object.keys(flags)) {
    if (!CALLER_FLAG_ALLOWLIST.has(key)) continue;
    out[key] = !!flags[key];
  }
  return out;
}
  ...
  try {
    proposal = await governFn(p.action_id, {
      ...
      params: {
        ...
        // Server-set flags first, then ONLY allowlisted caller flags. Order
        // matters: `safeFlags` cannot override `channel`/`delivers` because
        // neither is in the allowlist, so the platform's own description of
        // the action is authoritative.
        flags: { channel: spec.adapter.channel, delivers: !!spec.delivers, ...safeFlags(p.flags) },
      },
    });
  } catch (error) {
    await submitEvidenceOrFlag({
  ...
  try {
    proposal = await governFn(p.action_id, {
      ...
      params: {
        ...
        // As above: `reads`/`delivers` describe the operation and are not
        // caller-overridable, so a read cannot be relabelled as a non-read.
        flags: { channel: spec.adapter.channel, reads: true, delivers: false, ...safeFlags(p.flags) },
      },
    });
  } catch (error) {
    await submitEvidenceOrFlag({
...
async function submitEvidence({ org_id, environment_id, type, evidence, actor = "customer" }) {
  ...
  const event = await store.insert("integration_events", {
    ...
    actor: clean(actor, 160), evidence: evidence || {},
    evidence_hash: canonicalEvidenceHash(evidence), evidence_hash_alg: EVIDENCE_HASH_ALG,
...
async function overview(org_id = null) {
  ...
  const [connectors, webhooks, deliveries, deployments, usage, events, orgs, decisionMetrics] = await Promise.all([
    ...
    // Database-side aggregation avoids transferring up to 100,000 immutable
    // decision rows merely to display one count in the Control Room.
    store.aggregate(org_id ? { org_id } : {}).catch(() => ({ total: 0 })),
  ]);
  ...
  return {
    ...
    governance_decisions: Number(decisionMetrics && decisionMetrics.total || 0),
    evidence_generated: Number(decisionMetrics && decisionMetrics.total || 0) + events.length + deliveries.length,
  };
}
...
module.exports = {
  ...
  canonicalJson, canonicalEvidenceHash, EVIDENCE_HASH_ALG,
  safeFlags, CALLER_FLAG_ALLOWLIST,
};
