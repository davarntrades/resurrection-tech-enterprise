...
const CONNECTOR_DEFINITIONS = Object.freeze([
  ...
  ["aws", "AWS"], ["azure", "Azure"], ["google_cloud", "Google Cloud"],
...
async function stageSecret(org_id, value, purpose = "integration") {
  const row = await store.insert("integration_secrets", {
    org_id, purpose: clean(purpose, 80), value_encrypted: seal(value),
...
async function createConnectorRaw(p) {
  ...
  const row = await store.insert("integration_connectors", {
    ...
    config: p.config || {}, secret_encrypted: seal(staged || null),
...
module.exports = {
  CONNECTOR_DEFINITIONS, SDK_METHODS, connectorDefinition, safeEndpoint, assertEndpointNetwork,
