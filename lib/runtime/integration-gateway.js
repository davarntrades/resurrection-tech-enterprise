...
async function communicationConnectorRow({ org_id, environment_id, connector_id, connector_type }) {
  ...
  if (row.health !== "healthy") throw new Error("communication connector is not healthy");
  return row;
}
...
async function checkCommunicationHealthRaw(p, dependencies = {}) {
  ...
  const row = await communicationConnectorRow({ ...p, connector_type: adapter.connector_type });
  const started = Date.now();
  try {
...
module.exports = {
  ...
  sendCommunication, checkCommunicationHealthRaw, communicationConnectorRow,
