...
/**
 * The single governance entry point for every connector.
 *
 * Without a session this is byte-identical to the proposal it has always
 * created — same action, params, lifecycle, evidence — which is what lets the
 * Bedrock and communication paths migrate onto the step middleware with no
 * external behaviour change.
 *
 * With `params.session_id` the SAME call additionally becomes a governed STEP:
 * the accumulated workflow trajectory is evaluated at the configured horizon
 * and the more restrictive verdict wins. That is how a connector invocation
 * inside a multi-step agent workflow gets reachability governance for free,
 * without the connector knowing anything about sessions.
 */
async function governed(action_id, { org_id, environment_id = null, params = {}, actor = "integration_gateway", session_id = null }) {
  const ops = require("../ops");
  const session = session_id || params.session_id || null;
  if (!session) {
  ...
  const steps = require("./step-governance");
  const { session_id: _drop, ...stepParams } = params;
  const step = await steps.governStep(action_id, {
    session_id: session, org_id, environment_id, actor, params: stepParams,
  });
  // Connectors branch on `executed(proposal)`, so a step the trajectory gate
  // refused must not present as an executed proposal. Reshaping it here keeps
  // every existing connector's fail-closed logic correct and unmodified.
  const proposal = await ops.proposals.get(step.proposal_id);
  if (step.allowed) return proposal;
  return {
    ...proposal,
    status: step.verdict === "escalate" ? "escalated" : "blocked",
    execution: null,
    decision: {
      ...(proposal && proposal.decision ? proposal.decision : {}),
      verdict: step.verdict,
      policy: step.restricted_by_trajectory ? "trajectory_reachability" : (proposal && proposal.decision && proposal.decision.policy) || null,
      rule: step.trajectory_rule || (proposal && proposal.decision && proposal.decision.rule) || null,
      reason: step.restricted_by_trajectory
        ? "the accumulated workflow trajectory reaches a forbidden state; the step was blocked before execution"
        : (proposal && proposal.decision && proposal.decision.reason) || null,
    },
  };
}
