...
async function createConnectorRaw(p) {
  ...
  if (def.id === "salesforce" || def.id === "servicenow") {
    const provider = require(`./connectors/${def.id}`);
    config = provider.publicConfiguration(provider.validateConfiguration(config, staged || {}));
  }
...
async function checkConnectorHealthRaw(p) {
  ...
  if (row.type === "salesforce" || row.type === "servicenow") {
    return checkEnterpriseConnectorHealthRaw({ ...p, connector_type: row.type });
  }
...
function governanceShape(proposal) {
  const decision = proposal && proposal.decision;
  return {
    ...
    // Replay spine. The engine already returns a trajectory hash and an
    // attestation, and lib/ops/evidence.js writes them into the immutable
    // evidence row — dropping them here was the only reason a governed run
    // could not report how to reproduce its own verdict.
    verdict: decision && decision.verdict || null,
    rule: decision && decision.rule || null,
    omega_domain: decision && decision.omega_domain || null,
    trajectory_hash: decision && decision.trajectory_hash || null,
    engine_attestation: decision && decision.engine && decision.engine.attestation || null,
  };
}
...
async function checkCommunicationHealthRaw(p, dependencies = {}) {
  ...
  try {
    ...
    const persisted = await updateConnectorHealth(row.id, { status: "healthy", latency_ms: Date.now() - started, error: null });
    return { ok: true, connector_id: row.id, provider: adapter.provider, mailbox: result.mailbox, latency_ms: Date.now() - started, health: persisted.health, last_checked_at: persisted.last_checked_at };
  } catch (error) {
    const mapped = adapter.load().mapError(error);
    const persisted = await updateConnectorHealth(row.id, { status: mapped.retryable ? "degraded" : "down", latency_ms: Date.now() - started, error: `${mapped.code}: ${mapped.message}` });
    return { ok: false, connector_id: row.id, code: mapped.code, error: mapped.message, health: persisted.health, last_checked_at: persisted.last_checked_at };
  }
}

async function enterpriseConnectorRow({ org_id, environment_id, connector_id, connector_type, require_healthy = true }) {
  const row = assertOrg(await store.findOne("integration_connectors", { id: connector_id }), org_id, "enterprise connector");
  if (row.type !== connector_type) throw new Error(`connector is not a ${connector_type} connector`);
  if (row.environment_id !== environment_id) throw new Error("connector environment mismatch");
  if (row.status === "disabled") throw new Error("enterprise connector is disabled");
  if (require_healthy && row.health !== "healthy") throw new Error("enterprise connector is not healthy");
  return row;
}

/** Live OAuth + lightweight provider probe. Failure is persisted and surfaced. */
async function checkEnterpriseConnectorHealthRaw(p, dependencies = {}) {
  const adapters = require("./enterprise-action-adapters");
  const adapter = adapters.adapterFor(p.connector_type);
  const row = await enterpriseConnectorRow({ ...p, connector_type: adapter.connector_type, require_healthy: false });
  const started = Date.now();
  try {
    const identity = await adapter.load().validateCredentials(row.config || {}, connectorSecret(row), dependencies);
    const timestamp = store.nowISO();
    await store.update("integration_connectors", row.id, {
      config: adapter.load().publicConfiguration({
        ...(row.config || {}),
        ...(identity.instance_url ? { instance_url: identity.instance_url } : {}),
        authenticated_identity: identity.identity || null,
        credential_validated_at: timestamp,
      }),
      health: "healthy", last_checked_at: timestamp, last_error: null,
      latency_ms: Date.now() - started,
    });
    return {
      ok: true, connector_id: row.id, provider: adapter.provider,
      identity: identity.identity || null, latency_ms: Date.now() - started,
      health: "healthy", last_checked_at: timestamp,
    };
  } catch (error) {
    const mapped = adapter.load().mapError(error);
    const persisted = await updateConnectorHealth(row.id, {
      status: mapped.retryable ? "degraded" : "down",
      latency_ms: Date.now() - started, error: `${mapped.code}: ${mapped.message}`,
    });
    return { ok: false, connector_id: row.id, provider: adapter.provider, code: mapped.code, error: mapped.message, health: persisted.health, last_checked_at: persisted.last_checked_at };
  }
}

/** Validate replacement OAuth credentials live before atomically resealing. */
async function rotateEnterpriseCredentialsRaw(p, dependencies = {}) {
  const adapters = require("./enterprise-action-adapters");
  const adapter = adapters.adapterFor(p.connector_type);
  const row = await enterpriseConnectorRow({ ...p, connector_type: adapter.connector_type, require_healthy: false });
  const replacement = await consumeStagedSecret(p.org_id, p.secret_ref);
  const merged = { ...(row.config || {}), ...(p.config || {}) };
  const validated = adapter.load().validateConfiguration(merged, replacement || {});
  const identity = await adapter.load().validateCredentials(validated, replacement || {}, dependencies);
  const timestamp = store.nowISO();
  await store.update("integration_connectors", row.id, {
    config: adapter.load().publicConfiguration({
      ...validated,
      ...(identity.instance_url ? { instance_url: identity.instance_url } : {}),
      authenticated_identity: identity.identity || null,
      credential_validated_at: timestamp, credential_rotated_at: timestamp,
    }),
    secret_encrypted: seal(replacement || {}), health: "healthy",
    last_checked_at: timestamp, last_error: null,
  });
  return publicConnector(await store.findOne("integration_connectors", { id: row.id }));
}

/**
 * Provider-neutral governed CRM/ITSM action.
 * The proposal executor authorizes only. This function reaches the provider
 * exclusively when the proposal itself is executable.
 */
async function executeEnterpriseAction(p, dependencies = {}) {
  const adapters = require("./enterprise-action-adapters");
  const spec = adapters.operationFor(p.action_id);
  const row = await enterpriseConnectorRow({ ...p, connector_type: spec.adapter.connector_type });
  const normalised = adapters.normaliseInput(p.action_id, row.config || {}, p.input || {});
  const payloadHash = adapters.payloadHash(p.action_id, row.config || {}, p.input || {});
  const governFn = dependencies.governed || governed;
  let proposal;
  try {
    proposal = await governFn(p.action_id, {
      org_id: p.org_id, environment_id: p.environment_id, actor: p.actor || "enterprise_gateway",
      params: {
        canonical_action: p.canonical_action || { action_id: p.action_id },
        enterprise_action_run_id: p.enterprise_action_run_id || null,
        connector_id: row.id, payload_hash: payloadHash,
        target_type: normalised.object || normalised.table || null,
        target_record_id: normalised.record_id || null,
        flags: {
          enterprise_record_action: true, provider: spec.adapter.provider,
          reads: !!spec.reads, mutates: !!spec.mutates, destination_external: !!spec.mutates,
          ...(p.flags || {}),
        },
      },
    });
    ...
    await submitEvidence({
      org_id: p.org_id, environment_id: p.environment_id,
      type: "enterprise.action.governance.unavailable", actor: p.actor || "enterprise_gateway",
      evidence: { connector_id: row.id, action_id: p.action_id, payload_hash: payloadHash, outcome: "BLOCK" },
    }).catch(() => {});
    return { ok: false, code: "GOVERNANCE_UNAVAILABLE", error: "Runtime Governance unavailable; provider action blocked", payload_hash: payloadHash, governance: { status: "blocked" } };
  }
  const governance = governanceShape(proposal);
  if (!executed(proposal)) {
    const code = proposal.status === "escalated" ? "GOVERNANCE_ESCALATED" : "GOVERNANCE_BLOCKED";
    await submitEvidence({
      org_id: p.org_id, environment_id: p.environment_id,
      type: "enterprise.action.governance.decision", actor: p.actor || "enterprise_gateway",
      evidence: {
        connector_id: row.id, action_id: p.action_id, payload_hash: payloadHash,
        proposal_id: proposal.id, governance_evidence_id: proposal.evidence_id || null,
        outcome: proposal.status,
      },
    }).catch(() => {});
    return { ok: false, code, error: governance.reason || code, payload_hash: payloadHash, governance };
  }
  const started = Date.now();
  try {
    const result = await (dependencies.enterpriseExecute || adapters.execute)(
      p.action_id, row.config || {}, connectorSecret(row), p.input || {}, dependencies);
    const timestamp = store.nowISO();
    await store.update("integration_connectors", row.id, {
      config: { ...(row.config || {}), last_successful_request: timestamp },
      health: "healthy", last_checked_at: timestamp, last_error: null, latency_ms: Date.now() - started,
    ...
    await recordUsage({
      org_id: p.org_id, environment_id: p.environment_id, key_id: p.key_id || null,
      operation: `${spec.adapter.provider}.${spec.operation}`, sdk: p.sdk || null,
      latency_ms: Date.now() - started,
      meta: { connector_id: row.id, action_id: p.action_id, payload_hash: payloadHash, external_record_id: result.external_record_id || null },
    });
    const evidence = await submitEvidence({
      org_id: p.org_id, environment_id: p.environment_id,
      type: spec.mutates ? "enterprise.record.mutated" : "enterprise.record.read",
      actor: p.actor || "enterprise_gateway",
      evidence: {
        connector_id: row.id, action_id: p.action_id, provider: spec.adapter.provider,
        ...
        record_count: result.record_count == null ? null : Number(result.record_count),
        outcome: "success",
      },
    });
    return { ...result, ok: true, provider_invoked: true, payload_hash: payloadHash, governance, evidence };
  } catch (error) {
    const mapped = adapters.mapError(p.action_id, error);
    await updateConnectorHealth(row.id, {
      status: mapped.retryable ? "degraded" : "down", latency_ms: Date.now() - started,
      error: `${mapped.code}: ${mapped.message}`,
    });
    const evidence = await submitEvidence({
      org_id: p.org_id, environment_id: p.environment_id,
      type: "enterprise.action.failed", actor: p.actor || "enterprise_gateway",
      evidence: {
        connector_id: row.id, action_id: p.action_id, payload_hash: payloadHash,
    ...
    return {
      ok: false, code: mapped.code, error: mapped.message, retryable: !!mapped.retryable, provider_invoked: true,
      payload_hash: payloadHash, governance, evidence,
    };
  }
}
...
module.exports = {
  ...
  executeEnterpriseAction, checkEnterpriseConnectorHealthRaw, rotateEnterpriseCredentialsRaw, enterpriseConnectorRow,
  ...
  createDeploymentRaw, listDeployments, recordUsage, submitEvidence, governed, overview,
};
