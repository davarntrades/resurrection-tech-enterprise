...
async function listExecutionRecords(filter = {}) {
  ...
  return rows.filter((row) =>
    (!filter.environment_id || row.environment_id === filter.environment_id)
    && (!filter.since || String(row.created_at) >= String(filter.since))
    && (!filter.until || String(row.created_at) < String(filter.until)))
    .sort((a, b) => String(b.created_at).localeCompare(String(a.created_at))).slice(0, filter.limit || 100)
