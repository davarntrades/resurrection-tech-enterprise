...
function defaultClient({ endpoint, credentials, fetchImpl = global.fetch }) {
  return {
    async execute(request = {}) {
      if (typeof fetchImpl !== "function") throw new Error("Google network client is unavailable");
      const url = new URL(String(request.path || ""), endpoint);
      if (credentials.api_key) url.searchParams.set("key", String(credentials.api_key));
      ...
      try {
        ...
        const safe = safeError(error, "Google provider request failed");
        const wrapped = new Error(safe.message); wrapped.code = safe.code || "GOOGLE_REQUEST_FAILED"; throw wrapped;
      }
    },
  };
}

async function execute({ service = "vertex", config = {}, credentialRegistry, credentialReference, credentialContext = {}, outboundPolicy, governance, proposal, request = {}, clientFactory, fetchImpl }) {
  ...
  const prepared = await prepareProviderCall({ connectorType: "google-vertex-ai", config, service, defaultEndpoint: providerDefaults[service], credentialRegistry, credentialReference, credentialContext: context, outboundPolicy, governance, proposal, purpose: service === "identity" ? "identity" : "provider_execution" });
