 ...
 *
 * AWS-specific authentication, payload mapping and SDK calls live here. This
 * module never decides whether an operation is allowed: callers must obtain an
 * executed GuardianOS proposal before invoking any function that reaches AWS.
 * ============================================================================ */
"use strict";
...
function validateConfiguration(config = {}, secret = {}, { allowAmbient = true } = {}) {
  ...
  if (role_arn && !ROLE_ARN.test(role_arn))
    throw new BedrockConnectorError("AWS_INVALID_ROLE_ARN", "IAM role ARN is invalid", { status: 400 });
  if (auth_method === "access_key" && (!secret.access_key_id || !secret.secret_access_key))
    throw new BedrockConnectorError("AWS_MISSING_CREDENTIALS", "access key ID and secret access key are required", { status: 400 });
  if (auth_method === "role" && !role_arn)
    throw new BedrockConnectorError("AWS_MISSING_ROLE", "IAM role ARN is required for role authentication", { status: 400 });
  if (auth_method === "role" && !allowAmbient && (!secret.access_key_id || !secret.secret_access_key))
    throw new BedrockConnectorError("AWS_MISSING_SOURCE_CREDENTIALS", "source credentials are required in this deployment to assume the IAM role", { status: 400 });
  if ((secret.access_key_id && !secret.secret_access_key) || (!secret.access_key_id && secret.secret_access_key))
    throw new BedrockConnectorError("AWS_INCOMPLETE_CREDENTIALS", "access key ID and secret access key must be supplied together", { status: 400 });
  return {
    region,
    auth_method,
    role_arn,
...
function publicConfiguration(config = {}) {
  const placeholder = config.auth_method === "access_key"
...
async function resolvedCredentials(config, secret = {}, overrides = {}) {
  const d = dependencies(overrides);
  const base = staticCredentials(secret);
  if (!config.role_arn) return base;
  const sts = new d.STSClient({ region: config.region, ...(base ? { credentials: base } : {}), maxAttempts: 1 });
  let response;
  try {
    response = await sts.send(new d.AssumeRoleCommand({
      RoleArn: config.role_arn,
      RoleSessionName: `GuardianOS-${Date.now()}`,
      DurationSeconds: 3600,
      ...(secret.external_id ? { ExternalId: String(secret.external_id) } : {}),
    }));
  } catch (error) {
    const mapped = mapError(error);
    throw new BedrockConnectorError("AWS_ROLE_ASSUMPTION_FAILED", mapped.message, {
      status: mapped.status, retryable: mapped.retryable,
      aws_request_id: mapped.aws_request_id, cause_name: mapped.cause_name,
    });
  }
  const c = response && response.Credentials;
  if (!c || !c.AccessKeyId || !c.SecretAccessKey || !c.SessionToken)
    throw new BedrockConnectorError("AWS_ROLE_ASSUMPTION_FAILED", "AWS STS did not return temporary role credentials", { status: 502 });
...
async function validateCredentials(configInput, secret = {}, overrides = {}) {
  ...
  const sts = new d.STSClient({ region: config.region, ...(credentials ? { credentials } : {}), maxAttempts: 1 });
  try {
...
function assertResourceRegion(resource, region) {
  ...
  if (actual && actual !== region && actual !== "*")
    throw new BedrockConnectorError("AWS_REGION_MISMATCH", `resource region ${actual} does not match connector region ${region}`, { status: 400 });
}

function mapError(error, fallback = "AWS_BEDROCK_ERROR") {
  if (error instanceof BedrockConnectorError) return error;
  const name = clean(error && (error.name || error.Code || error.code), 120);
  const requestId = error && error.$metadata && error.$metadata.requestId;
  ...
  const code = table[name] || fallback;
  return new BedrockConnectorError(code, clean(error && error.message ? error.message : "Amazon Bedrock request failed", 500), {
    status: Number(status) || (code === "AWS_INVALID_CREDENTIALS" || code === "AWS_ACCESS_DENIED" ? 401 : 502),
    retryable: RETRYABLE.has(name),
    aws_request_id: requestId || null,
  ...
  while (true) {
    ...
    try {
      ...
      const error = raw && raw.name === "AbortError"
        ? new BedrockConnectorError("AWS_TIMEOUT", `Amazon Bedrock request exceeded ${config.timeout_ms}ms`, { status: 504, retryable: true })
        : mapError(raw);
...
function invocationInput(request = {}, config) {
  ...
  if (mode === "converse") {
    if (!Array.isArray(request.messages) || !request.messages.length)
      throw new BedrockConnectorError("AWS_MESSAGES_REQUIRED", "Converse requests require messages", { status: 400 });
    return {
      mode, stream: !!request.stream,
      input: {
        modelId, messages: request.messages,
        ...(request.system ? { system: request.system } : {}),
        ...(request.inference_config ? { inferenceConfig: request.inference_config } : {}),
        ...(request.tool_config ? { toolConfig: request.tool_config } : {}),
        ...(request.additional_model_request_fields ? { additionalModelRequestFields: request.additional_model_request_fields } : {}),
      },
    };
  }
  return {
...
async function collectStream(stream) {
  ...
  for await (const event of stream) {
    if (event.chunk && event.chunk.bytes) {
      ...
      let body = text; try { body = JSON.parse(text); } catch { /* provider-specific text */ }
...
async function invoke(configInput, secret, request, overrides = {}) {
  ...
  const d = dependencies(overrides);
  const credentials = await resolvedCredentials(config, secret, overrides);
  const client = new d.BedrockRuntimeClient({
  ...
  const make = () => {
    if (mapped.mode === "converse")
      return mapped.stream ? new d.ConverseStreamCommand(mapped.input) : new d.ConverseCommand(mapped.input);
    return mapped.stream ? new d.InvokeModelWithResponseStreamCommand(mapped.input) : new d.InvokeModelCommand(mapped.input);
  };
  try {
    ...
    if (mapped.stream) {
      ...
      return { ok: true, mode: mapped.mode, stream: true, events: await collectStream(stream), attempts, aws_request_id: requestId || null };
    }
    if (mapped.mode === "converse") {
      return {
        ok: true, mode: "converse", output: result.output || null, stop_reason: result.stopReason || null,
        usage: result.usage || null, metrics: result.metrics || null, attempts, aws_request_id: requestId || null,
      };
    }
    const text = result && result.body ? Buffer.from(result.body).toString("utf8") : "";
    let body = text; try { body = text ? JSON.parse(text) : null; } catch { /* provider-specific body */ }
    return { ok: true, mode: "invoke", body, content_type: result.contentType || null, attempts, aws_request_id: requestId || null };
  } catch (error) {
...
function mapActionGroupEvent(input, context = {}) {
  ...
  if (!event.actionGroup || (!event.apiPath && !event.function))
    throw new BedrockConnectorError("AWS_MALFORMED_PAYLOAD", "actionGroup and apiPath or function are required", { status: 400 });
  const agent = event.agent || {};
  const parameters = Array.isArray(event.parameters) ? event.parameters.map((p) => ({
    name: clean(p && p.name, 200), type: clean(p && p.type, 80), value: clean(p && p.value, 2000),
  })) : [];
  ...
  const path = clean(event.apiPath || event.function, 500);
  return {
    ...
    agent: {
      id: clean(agent.id, 200) || null, alias: clean(agent.alias, 200) || null,
      name: clean(agent.name, 200) || null, version: clean(agent.version, 80) || null,
    },
    action_group: clean(event.actionGroup, 300),
    api_path: path, http_method: method, function_name: event.function ? clean(event.function, 300) : null,
    parameters, request_body: requestBody,
...
function actionGroupResponse(canonical, outcome, result = null) {
  ...
  const body = {
    ...
    proposal_id: result && result.proposal_id || null,
    evidence_id: result && result.evidence_id || null,
    reason: result && result.reason || null,
    ...(decision === "PERMIT" && result && result.action_result !== undefined ? { result: result.action_result } : {}),
  };
  const response = canonical.function_name ? {
    actionGroup: canonical.action_group,
    function: canonical.function_name,
    functionResponse: {
      ...(decision === "BLOCK" ? { responseState: "FAILURE" } : decision === "ESCALATE" ? { responseState: "REPROMPT" } : {}),
      responseBody: { TEXT: { body: JSON.stringify(body) } },
    },
  } : {
    actionGroup: canonical.action_group,
    apiPath: canonical.api_path,
    httpMethod: canonical.http_method,
    httpStatusCode: status,
    responseBody: { "application/json": { body: JSON.stringify(body) } },
  };
  return {
    messageVersion: canonical.message_version || "1.0",
    response,
    sessionAttributes: canonical.session_attributes || {},
    promptSessionAttributes: canonical.prompt_session_attributes || {},
  };
}
...
function verifyInboundSignature({ secret, timestamp, nonce, signature, rawBody, toleranceSeconds = 300, now = Date.now() }) {
  ...
  if (!Number.isFinite(seconds) || Math.abs(Math.floor(now / 1000) - seconds) > toleranceSeconds)
    return { ok: false, code: "AWS_SIGNATURE_EXPIRED" };
  ...
  return { ok: a.length === b.length && crypto.timingSafeEqual(a, b), code: a.length === b.length && crypto.timingSafeEqual(a, b) ? null : "AWS_SIGNATURE_INVALID", nonce_hash: hash(String(nonce)) };
}

module.exports = {
  BedrockConnectorError, validateConfiguration, publicConfiguration, validateCredentials,
  resolvedCredentials, resourceRegion, assertResourceRegion, mapError, sendWithRetry,
  invocationInput, invoke, unwrapLambdaEvent, mapActionGroupEvent, actionGroupResponse,
  verifyInboundSignature, hash,
};
