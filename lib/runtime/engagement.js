...
function shape(r, org_id) {
  if (!r) return { org_id, stage: "prospect", stage_label: "Prospect", next_review_date: null, cadence: "monthly", delivery_schedule: "", contacts: [], notes: [], last_review_date: null, configured: false, updated_at: null };
  const stage = norm(r.stage, STAGE_KEYS, "prospect");
  return {
    org_id: r.org_id,
    stage,
...
async function ensure(org_id) {
  ...
  if (!row) row = await store.insert("engagements", { org_id, stage: "prospect", cadence: "monthly", delivery_schedule: "", contacts: [], notes: [], next_review_date: null, last_review_date: null, updated_at: store.nowISO() });
  return row;
}
...
module.exports = { CADENCES, STAGES, STAGE_KEYS, stageLabel, get, set, addContact, removeContact, addNote, dueForReview };
