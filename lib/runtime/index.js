...
const baseGateway = require("./gateway");
const gateway = require("./production-governance-runtime").wrapGovernanceGateway(baseGateway, store);
const metrics = require("./metrics");
const baseReports = require("./reports");
const reports = require("./production-reports-runtime").wrapReports(baseReports, store, productionReadiness);
...
const sovereignIntegrationGateway = require("./sovereign/integration-gateway-runtime").wrapIntegrationGateway(baseIntegrationGateway, store);
const integrationGateway = require("./production-gateway-runtime").wrapProductionGateway(sovereignIntegrationGateway, store);
const approvedBedrock = require("./bedrock-approved-invocation");
integrationGateway.executeApprovedBedrockInvocation = async (input, dependencies) => {
  await integrationGateway.productionExecutionGate(input || {});
  return approvedBedrock.executeApprovedBedrockInvocation(input, integrationGateway, dependencies);
};
const approvedCommunication = require("./communication-approved-send");
integrationGateway.executeApprovedCommunication = async (input, dependencies) => {
  await integrationGateway.productionExecutionGate(input || {});
  return approvedCommunication.executeApprovedCommunication(input, integrationGateway, dependencies);
};
...
async function health() {
  ...
  return {
    status: eng.ok ? "ok" : "degraded", time: store.nowISO(),
    engine: {
    ...
    store: (() => {
      ...
      return {
        backend: store.backend(), durable: store.durable(), profile: prof.id,
        data_dir: local ? store.DATA_DIR : null,
        cloud_refused: store.cloudRefused(), cloud_error: store.cloudError(), fault: store.storageFault(),
        warning: !local ? null
