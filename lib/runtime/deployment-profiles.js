...
async function evaluate({ org_id, environment_id, profile, config = {} }) {
  ...
  if (p === PROFILE.SOVEREIGN) {
    return readiness.sovereignReadiness({
      ...
      customer_evidence_store: config.evidence_store_ref,
      sovereign_profile: config.sovereign_profile || SOVEREIGN_DEFAULTS.sovereign_profile,
    });
  }
  return {
    status: readiness.POSTURE.READY, posture: readiness.POSTURE.READY, ready: true,
    profile: p, checked_at: new Date().toISOString(),
...
async function preflight(input) {
  ...
  if (existing) await store.update("deployment_profiles", existing.id || input.environment_id, {
    last_preflight: result, last_verified_at: result.checked_at || store.nowISO(),
    status: result.ready ? "ready" : "blocked", updated_at: store.nowISO(),
  });
  return result;
}
