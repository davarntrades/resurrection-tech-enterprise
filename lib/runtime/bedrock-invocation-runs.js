...
async function acquireExecutionLock(run) {
  ...
  try {
    ...
    // 23505 is PostgreSQL's unique_violation; the message match covers the
    // PostgREST/file-store phrasings. A lost race is not an error — it means
    // another caller already owns execution for this run.
    if (error && error.code === "23505") return false;
    if (/duplicate|unique|already exists|23505/i.test(String(error && error.message || error))) return false;
    throw error;
  }
  // Supabase enforces the unique run_id constraint. The hermetic JSON store does
  // not, so verify that this caller owns the first durable lock record. This
  // preserves the exact same at-most-once semantics in local/sovereign tests.
...
function resultPatch(result, total) {
...
async function executeRun(run, integrationGateway, dependencies = {}) {
  ...
  await store.update("bedrock_invocation_runs", run.id, resultPatch(result, Date.now() - totalStarted));
...
async function reconcileApproval(run, integrationGateway, dependencies = {}) {
  ...
  if (proposal.status === "denied" || proposal.status === "blocked") {
    await store.update("bedrock_invocation_runs", run.id, {
      ...
      governance_decision: proposal.status,
      evidence_id: proposal.evidence_id || run.evidence_id || null,
      evidence_count: proposal.evidence_id ? 1 : Number(run.evidence_count || 0),
      provider_invocation_count: 0,
      aws_called: false,
      safe_failure_reason: proposal.status === "denied" ? "Approval rejected; Amazon Bedrock was not called" : "Runtime Governance blocked execution after approval; Amazon Bedrock was not called",
      completed_at: now(),
      updated_at: now(),
    });
    return safeRun(await store.findOne("bedrock_invocation_runs", { id: run.id }));
  ...
  if (typeof integrationGateway.executeApprovedBedrockInvocation !== "function") {
    await store.update("bedrock_invocation_runs", run.id, {
      status: "failed",
      lifecycle_state: "complete",
      approval_status: "approved_but_execution_unavailable",
      provider_invocation_count: 0,
      aws_called: false,
      safe_failure_reason: "Approved execution continuation is unavailable; failed closed before Amazon Bedrock",
      completed_at: now(),
      updated_at: now(),
    });
    return safeRun(await store.findOne("bedrock_invocation_runs", { id: run.id }));
  ...
  await store.update("bedrock_invocation_runs", run.id, {
    status: "executing",
    lifecycle_state: "invoking_bedrock",
    approval_status: "approved",
    updated_at: now(),
  });
  let result;
  try {
    result = await integrationGateway.executeApprovedBedrockInvocation({
      org_id: run.org_id,
      environment_id: run.environment_id,
      connector_id: run.connector_id,
      model_id: run.model_id,
      proposal_id: run.proposal_id,
      request_hash: proposal.params && proposal.params.request_hash,
      request: requestFor(run),
      actor: run.actor,
    }, dependencies);
  } catch (error) {
  ...
  const patch = resultPatch(result, Date.now() - started);
  patch.approval_status = result && result.ok ? "approved_and_executed" : "approved_execution_failed";
  // An approved run accumulates evidence across both legs: the escalation
  // decision recorded before sign-off, plus the approval/provider evidence
  // recorded now. resultPatch only knows about this leg, so add rather than
  // overwrite — otherwise the console under-reports the evidence it holds.
...
module.exports = {
  ...
  listEligibleConnectors, createRuns, executeRun, reconcileApproval, advanceBatch, recentRuns, aggregate, safeRun,
};
