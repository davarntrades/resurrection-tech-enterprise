...
function classifyFailure(code) {
  ...
  if (/VALIDATION|INVALID_REGION|INVALID_ROLE|MODEL_REQUIRED|MESSAGES_REQUIRED|MALFORMED|REGION_MISMATCH|MODEL_NOT_ALLOWED/.test(value)) return "validation";
...
function resultPatch(result, timings = {}) {
  ...
  const providerLatency = providerCalled ? (recordedNumber(result && (result.provider_latency_ms ?? result.latency_ms)) ?? recordedNumber(timings.provider_latency_ms)) : recordedNumber(result && result.provider_latency_ms);
  ...
  const failureCategory = result && result.error_category || classifyFailure(failureCode);
  const failureMessage = clean(result && (result.error || result.code) || "Invocation failed closed", 430);
  const patch = {
    ...
    safe_failure_reason: result && result.ok ? null : clean(`${failureCategory} | ${failureCode}: ${failureMessage}`, 500),
...
async function executeRun(run, integrationGateway, dependencies = {}) {
  ...
  try {
    ...
    result = {
      ok: false,
      code,
      error: error && error.message || "Invocation orchestration failed closed",
      error_category: classifyFailure(code),
      governance: { status: code === "GOVERNANCE_UNAVAILABLE" ? "blocked" : "error" },
    };
  }
...
async function reconcileApproval(run, integrationGateway, dependencies = {}) {
  ...
  if (typeof integrationGateway.executeApprovedBedrockInvocation !== "function") {
    await store.update("bedrock_invocation_runs", run.id, { status: "failed", lifecycle_state: "complete", approval_status: "approved_but_execution_unavailable", provider_invocation_count: 0, aws_called: false, approval_wait_latency_ms: approvalWait, safe_failure_reason: "internal_orchestration | INTERNAL_ORCHESTRATION_ERROR: Approved execution continuation is unavailable; failed closed before Amazon Bedrock", completed_at: now(), updated_at: now() });
    return safeRun(await store.findOne("bedrock_invocation_runs", { id: run.id }));
  }
  ...
  try {
    ...
    const code = error && error.code || "INTERNAL_ORCHESTRATION_ERROR";
    result = { ok: false, code, error: error && error.message || "Approved execution failed closed", error_category: classifyFailure(code), governance: { proposal_id: run.proposal_id, status: proposal.status } };
  }
...
module.exports = { TERMINAL, MAX_REQUESTS, MAX_CONCURRENCY, listEligibleConnectors, createRuns, executeRun, reconcileApproval, advanceBatch, recentRuns, aggregate, safeRun, classifyFailure };