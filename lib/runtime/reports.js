...
async function connectorActivityFor({ org_id, environment_id, window: w }) {
  try {
    ...
    return {
      ...s,
      register: s.register.slice(0, REGISTER_CAP),
...
function connectorMarkdown(ca) {
  ...
  if ((ca.register || []).length) {
    ...
    if (ca.register_total > REGISTER_DISPLAY) L.push(`_Showing ${REGISTER_DISPLAY} of ${ca.register_total} records. Complete identifiers are preserved in the exported audit data._`, "");
    ...
    for (const row of ca.register.slice(0, REGISTER_DISPLAY)) {
  ...
  if ((ca.findings || []).length) {
    ...
    for (const f of ca.findings.slice(0, 50)) L.push(`| ${f.severity} | ${f.kind} | ${f.detail} |`);
    L.push("");
...
function connectorHtml(ca, esc) {
  ...
  if ((ca.register || []).length) {
    ...
    if (ca.register_total > REGISTER_DISPLAY) H.push(`<p>Showing ${REGISTER_DISPLAY} of ${ca.register_total} records. Complete identifiers are preserved in the exported audit data.</p>`);
...
module.exports = { PERIODS, REGISTER_CAP, REGISTER_DISPLAY, windowFor, generate, generateAllDue, listReports, getReport, summarize, toMarkdown, toHtml, connectorActivityFor, connectorMarkdown, connectorHtml };
