...
function executiveLine(period, s) {
  ...
  return `Over the ${period} window, Runtime Governance evaluated ${s.total.toLocaleString()} agent trajector${s.total === 1 ? "y" : "ies"} — `
    ...
    + (s.latency.engine_compute_ms.mean != null ? `Mean service-handler time ${s.latency.engine_compute_ms.mean}ms.` : "")
...
function toMarkdown(r) {
  ...
  L.push(`| Mean service handler | ${r.latency.engine_compute_ms.mean != null ? r.latency.engine_compute_ms.mean + " ms" : "—"} |`);
  L.push(`| p95 service handler | ${r.latency.engine_compute_ms.p95 != null ? r.latency.engine_compute_ms.p95 + " ms" : "—"} |`, "");
...
<div class="summary-card"><span class="card-label">Mean decision time</span><span class="card-value">${meanCompute != null ? `${meanCompute} ms` : "Not measured"}</span><span class="card-copy">Mean service-handler time across recorded decisions (whole /v1/govern request, not Ω engine compute alone).</span></div>
</div></section>
...
<tr><td>Mean / p95 service handler</td><td colspan="2">${((r.latency || {}).engine_compute_ms || {}).mean ?? "—"} / ${((r.latency || {}).engine_compute_ms || {}).p95 ?? "—"} ms</td></tr></table>
