...
function normalizePolicy(input = {}) {
  ...
  const approved = (input.approved_endpoints || []).map((endpoint) => validateEndpoint(endpoint, { allowPrivate: true }));
  return Object.freeze({ mode, approved_endpoints: approved, custom: input.custom || null });
}
...
async function authorize(policyInput, request = {}, governance = null) {
  ...
  const destination = validateEndpoint(request.url, { allowPrivate: true });
