...
async function recordEvidence(run, providerRun) {
  return store.insert("integration_events", {
    ...
    evidence: {
      ...
      execution_status: providerRun.status,
      ...
      org_id: run.org_id,
      environment_id: run.environment_id,
      total_latency_ms: providerRun.total_latency_ms,
      canonical_governance_latency_ms: run.governance_latency_ms,
      provider_governance_latency_ms: providerRun.governance_evaluation_latency_ms ?? providerRun.governance_latency_ms,
      provider_latency_ms: providerRun.provider_latency_ms,
      provider_invocation_count: providerRun.provider_invocation_count,
      aws_called: providerRun.aws_called,
      underlying_evidence_id: providerRun.evidence_id || null,
      completed_at: providerRun.completed_at || null,
    },
    evidence_hash: hash({ workflow_run_id: run.id, proposal_id: run.proposal_id, provider_proposal_id: providerRun.proposal_id, status: providerRun.status, completed_at: providerRun.completed_at }),
  });
}
...
async function advanceExecution(workflow_run_id, org_id, integrationGateway, dependencies = {}) {
  ...
  if (!run || run.org_id !== org_id || TERMINAL.has(run.status)) return base.safe(run);
  ...
  if (TERMINAL.has(run.status) || run.status === "awaiting_approval") return base.safe(run);
  ...
  let evidence = null;
  if (TERMINAL.has(providerRun.status) && !run.workflow_evidence_recorded) evidence = await recordEvidence(run, providerRun);
  const canonicalLatency = Number(run.governance_latency_ms || 0);
  const providerGovernance = Number(providerRun.governance_evaluation_latency_ms ?? providerRun.governance_latency_ms ?? 0);
  await store.update("customer_support_workflow_runs", run.id, {
    ...
    underlying_evidence_id: providerRun.evidence_id || null,
    evidence_id: evidence ? evidence.id : run.evidence_id || null,
    evidence_count: Number(providerRun.evidence_count || 0) + (evidence ? 1 : 0),
    workflow_evidence_recorded: evidence ? true : !!run.workflow_evidence_recorded,
    completed_at: TERMINAL.has(providerRun.status) ? (providerRun.completed_at || now()) : null,
    updated_at: now(),
  });
  return base.safe(await store.findOne("customer_support_workflow_runs", { id: run.id }));
}
