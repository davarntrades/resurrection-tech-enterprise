...
// A missing COLUMN is a different failure from a missing table: PostgREST
// reports PGRST204, Postgres 42703. Matched separately so an additive column
// migration can degrade without also swallowing genuine schema errors.
const MISSING_COLUMN = /(could not find the '.*' column|PGRST204|42703|column .* does not exist)/i;
function isMissingColumn(err) {
  return !!err && MISSING_COLUMN.test(String((err && err.message) || err));
}

// Timing fields added by the governance-timing migration. Nullable and
// additive: droppable when the migration is pending, and deliberately NOT part
// of chainCore, so dropping them cannot affect the decision hash chain.
const ADDITIVE_TIMING_FIELDS = [
  "decision_time_ms", "engine_time_ms", "trajectory_decision_time_ms",
  "eval_number", "stage_timings_ms",
];
const ADDITIVE_EVIDENCE_FIELDS = [
  "governed_result", "engine_evidence", "governance_layer", "execution_occurred",
];
function registerPendingTimingMigration() {
  _pendingMigrations.add(`${TABLE("decisions")}.governance_timing_columns`);
}
...
// Read-only RPC passthrough, used by the assurance panel to introspect database
// metadata (see supabase/assurance_status.sql). Deliberately NOT modelled on
// aggregate()/aggregateTrends(), which throw: a status surface that throws is a
// status surface that gets wrapped in a try/catch and reported as "fine".
//
// Returns a result object and never throws, so every failure mode — no cloud
// backend, function not deployed, permission denied, network fault — arrives at
// the caller as an explicit negative it has to render. There is no shape this
// can return that a caller could mistake for "the control is present".
const MISSING_FUNCTION = /(could not find the function|does not exist|schema cache|PGRST202|42883)/i;
async function rpcOptional(fn, args = {}) {
  const sb = supabase();
  if (!sb) return { ok: false, reason: "no_cloud_backend", detail: `store backend is ${backend()}`, data: null };
  try {
    const { data, error } = await sb.rpc(fn, args);
    if (error) {
      const msg = String(error.message || error);
      return { ok: false, reason: MISSING_FUNCTION.test(msg) ? "function_missing" : "rpc_error", detail: msg, data: null };
    }
    return { ok: true, reason: null, detail: null, data: data == null ? [] : data };
  } catch (e) {
    return { ok: false, reason: "rpc_threw", detail: (e && e.message) || String(e), data: null };
  }
}
...
async function appendDecision(row) {
  ...
  for (let attempt = 0; attempt < MAX_TRIES; attempt++) {
    ...
    if (sb) {
      let { error } = await sb.from(TABLE("decisions")).insert(rec);
      if (error && isMissingColumn(error)) {
        // An additive timing migration has not been applied to this project.
        // Recording the DECISION matters more than recording how long it took,
        // so drop the additive fields and retry rather than failing the write.
        //
        // This is the rg_provisioning incident generalised: an unapplied
        // additive migration must not take a live path down. The gap is
        // registered so it surfaces in pendingMigrations() instead of being
        // silently lost — degrade, but never quietly.
        registerPendingTimingMigration();
        const reduced = { ...rec };
        for (const k of [...ADDITIVE_TIMING_FIELDS, ...ADDITIVE_EVIDENCE_FIELDS]) delete reduced[k];
        // entry_hash covers only chainCore fields, none of which are additive,
        // so the chain is unaffected by stripping them.
        ({ error } = await sb.from(TABLE("decisions")).insert(reduced));
      }
      if (error) {
...
module.exports = {
  ...
  isMissingTable, pendingMigrations, rpcOptional, appendDecision, queryDecisions,
