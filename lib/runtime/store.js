...
async function appendDecision(row) {
  ...
  const prev_hash = head ? head.head_hash : GENESIS;
  ...
  // Advance the per-environment chain head. (Single-node atomic on the file
  // store / one process; a multi-node Supabase deployment must do this in a
  // transaction — documented in the schema.)
