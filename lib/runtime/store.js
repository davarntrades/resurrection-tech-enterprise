...
function supabase() {
  ...
  try {
    ...
    require("./log").error("cloud_store_unavailable", {
      error: _cloudError,
      note: "Supabase credentials are set and the profile permits a cloud store, but the client could not be constructed — this deployment is running on the LOCAL file store",
    });
  }
  return _sb;
}
