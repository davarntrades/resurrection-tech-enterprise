...
// Evidence-hash verification. The hash is recomputed from the STORED row and
// compared to what was recorded, so an altered evidence payload stops matching.
//
// It is deliberately three-valued, never two. A row written before canonical
// hashing cannot be recomputed — Postgres jsonb does not preserve key order, so
// the original insertion-order hash is unreproducible. Such a row is
// "unverifiable", NOT "tampered": a legacy serialisation and a real alteration
// are indistinguishable, and an audit trail that guesses in the accusing
// direction is worse than one that admits what it cannot prove.
function verifyEvidenceHash(row, gateway) {
  const stored = row && row.evidence_hash;
  if (!stored) return { state: "absent", verified: false };
  if (row.evidence_hash_alg !== gateway.EVIDENCE_HASH_ALG) return { state: "unverifiable", verified: false };
  const recomputed = gateway.canonicalEvidenceHash(row.evidence || {});
  return recomputed === stored
    ? { state: "verified", verified: true }
    : { state: "mismatch", verified: false, recomputed };
}
...
const EVIDENCE_FIELDS = Object.freeze([
  "connector_id", "connector_name", "proposal_id", "evidence_id", "governance_evidence_id",
  ...
  // Attribution and telemetry the governed workflows already put in evidence.
  // Omitting them meant the report discarded hashes and latencies that were
  // sitting in the record it was reading — identifiers and durations only,
  // never message bodies, prompts, responses or customer identity.
  "workflow_run_id", "provider_proposal_id", "underlying_evidence_id",
  "canonical_action_hash", "governance_decision", "execution_status", "approval_status",
  "provider_invocation_count", "aws_called", "completed_at",
  "total_latency_ms", "governance_latency_ms", "provider_latency_ms",
  "canonical_governance_latency_ms", "provider_governance_latency_ms",
]);
...
function indexRuns(rows, shape) {
  ...
  for (const row of rows) {
    const rec = shape(row);
    // A workflow can own several identifiers for the same execution: its own
    // proposal, the provider proposal it delegated to, its evidence, and the
    // provider's evidence. Index every one of them, or the join silently misses
    // and the row reports nulls for telemetry that was recorded all along.
    for (const key of [rec.proposal_id, rec.provider_proposal_id]) {
      if (key && !byProposal.has(key)) byProposal.set(key, rec);
    }
    for (const key of [rec.evidence_id, rec.underlying_evidence_id, rec.run_id]) {
      if (key && !byEvidence.has(key)) byEvidence.set(key, rec);
    }
  }
  return { byProposal, byEvidence };
}

// The Customer Support Assistant keeps its own run record. It is the canonical
// workflow above a provider invocation, so it — not the Bedrock run — holds the
// canonical action hash and the end-to-end latency for the governed business
// action. Omitting this table is why a traced execution reported null hashes.
function customerSupportRunShape(row) {
  return {
    source: "customer_support_workflow_run",
    run_id: row.id, proposal_id: row.proposal_id || null,
    provider_proposal_id: row.provider_proposal_id || null,
    evidence_id: row.evidence_id || null,
    underlying_evidence_id: row.underlying_evidence_id || null,
    connector_id: row.connector_id || null, connector_name: row.connector_name || null,
    model: row.model_id || null, provider: row.provider || null,
    ...
    request_hash: row.canonical_action_hash || null, response_hash: row.response_hash || null,
...
function bedrockRunShape(row) {
  return {
    ...
    provider_proposal_id: null, underlying_evidence_id: null,
    connector_id: row.connector_id || null, connector_name: row.connector_name || null,
...
async function project({ org_id, environment_id = null, since = null, until = null } = {}) {
  ...
  const [events, connectors, proposals, bedrockRuns, commRuns, supportRuns] = await Promise.all([
    ...
    store.findOptional("customer_support_workflow_runs", { org_id }),
  ]);
  ...
  // Workflow runs are consulted FIRST: the canonical business action owns the
  // action hash and the end-to-end latency, and the provider run it delegated
  // to only knows about its own leg.
  const support = indexRuns(supportRuns.filter(inScope), customerSupportRunShape);

  // Required lazily: the gateway owns the hashing contract, and importing it at
  // module load would create a cycle through the runtime barrel.
  const gateway = require("./integration-gateway");

  const inOrg = events.filter(inScope);
  // Evidence with no usable timestamp cannot be attributed to a period, so it
  // must not enter the register — that would corrupt every period total. But it
  // must not vanish either: unattributable evidence is a finding, because
  // silently dropping a record is the one thing an audit projection must never
  // do. This is the only path by which in-scope evidence leaves the register.
  const undated = inOrg.filter((e) => {
    const t = eventTime(e);
  ...
  for (const e of undated) {
    findings.push({
      kind: "evidence_without_usable_timestamp", severity: "high",
      evidence_id: e.id, connector_id: (e.evidence && e.evidence.connector_id) || null,
      detail: `${e.type} has no parseable occurred_at or created_at, so it cannot be attributed to a reporting period and is excluded from the register`,
    });
  }
  ...
  for (const event of scoped) {
    ...
    const integrity = verifyEvidenceHash(event, gateway);
    ...
    const workflowRunId = evidence.workflow_run_id || null;
    const providerProposalId = evidence.provider_proposal_id || null;
    const run =
      (workflowRunId && support.byEvidence.get(workflowRunId))
      || (proposal_id && (support.byProposal.get(proposal_id) || bedrock.byProposal.get(proposal_id) || comms.byProposal.get(proposal_id)))
      || (providerProposalId && (bedrock.byProposal.get(providerProposalId) || support.byProposal.get(providerProposalId)))
      || support.byEvidence.get(event.id) || bedrock.byEvidence.get(event.id) || comms.byEvidence.get(event.id)
      || null;
    ...
    const provider_invocation_count = (run && run.provider_invocation_count != null)
      ...
      : (num(evidence.provider_invocation_count) ?? (kind.provider_call ? 1 : 0));

    const row = {
      evidence_id: event.id,
      evidence_hash: event.evidence_hash || null,
      evidence_hash_state: integrity.state,
      evidence_hash_verified: integrity.verified,
      ...
      approval_status: (run && run.approval_status) || evidence.approval_status
      ...
      execution_status: (run && run.execution_status) || evidence.execution_status || evidence.status || null,
      provider_invocation_count,
      provider_called: run ? run.provider_called === true
        : (evidence.aws_called !== undefined ? evidence.aws_called === true : kind.provider_call),
      ...
      // Prefer the run record, then fall back to what the evidence itself
      // recorded. The evidence is written at execution time and already carries
      // hashes and latencies; reading only the run meant a workflow whose run
      // table was not consulted reported nulls for telemetry it had captured.
      request_hash: (run && run.request_hash) || evidence.request_hash || evidence.canonical_action_hash || null,
      response_hash: (run && run.response_hash) || evidence.response_hash || evidence.message_hash || null,
      governance_latency_ms: (run && run.governance_latency_ms)
        ?? num(evidence.governance_latency_ms) ?? num(evidence.canonical_governance_latency_ms) ?? null,
      provider_latency_ms: (run && run.provider_latency_ms) ?? num(evidence.provider_latency_ms) ?? null,
      total_latency_ms: (run && run.total_latency_ms) ?? num(evidence.total_latency_ms) ?? null,
    ...
    if (kind.outcome !== "control") {
      ...
      if (integrity.state === "mismatch") {
        findings.push({
          kind: "evidence_hash_mismatch", severity: "critical",
          evidence_id: event.id, connector_id,
          detail: `recorded evidence_hash does not match the stored payload — this evidence has been altered since it was written`,
        });
      } else if (integrity.state === "absent") {
        findings.push({
          kind: "evidence_hash_absent", severity: "high",
          evidence_id: event.id, connector_id,
          detail: `${event.type} carries no evidence_hash, so its integrity cannot be checked at all`,
        });
      }
...
async function summary({ org_id, environment_id = null, since = null, until = null } = {}) {
  ...
  const verifiedRows = rows.filter((r) => r.evidence_hash_verified).length;
  const unverifiable = rows.filter((r) => r.evidence_hash_state === "unverifiable").length;
  const mismatched = rows.filter((r) => r.evidence_hash_state === "mismatch").length;
  ...
  return {
    ...
    totals: {
      ...
      evidence_hash_verified: verifiedRows,
      evidence_hash_unverifiable: unverifiable,
      evidence_hash_mismatched: mismatched,
