...
// Production defaults to ENFORCE. Staging/sandbox may still be created in
// shadow explicitly; production may not silently be.
async function createEnvironment({ org_id, kind = "production", mode, name, store_payloads = false }) {
  if (!mode) mode = kind === "production" ? "enforce" : "shadow";
...
async function onboardCustomer({ name, slug, plan = "pilot" }) {
  ...
  if (!envs.length) {
    await createEnvironment({ org_id: org.id, kind: "production", mode: "enforce", name: "production" });
