...
create table if not exists public.rg_bedrock_invocation_runs (
  ...
  governance_evaluation_latency_ms integer,
...
-- Upgrade an already-created preview/production table safely. Historical rows
-- remain null so missing telemetry is never silently rewritten as zero.
...
alter table public.rg_bedrock_invocation_runs add column if not exists governance_started_at timestamptz;
alter table public.rg_bedrock_invocation_runs add column if not exists governance_completed_at timestamptz;
alter table public.rg_bedrock_invocation_runs add column if not exists governance_evaluation_latency_ms integer;
alter table public.rg_bedrock_invocation_runs add column if not exists approval_wait_latency_ms integer;
...
alter table public.rg_bedrock_invocation_runs add constraint rg_bedrock_invocation_runs_count_check check (requested_count between 1 and 10) not valid;
...
alter table public.rg_bedrock_invocation_runs add constraint rg_bedrock_invocation_runs_concurrency_check check (concurrency between 1 and 3) not valid;
...
alter table public.rg_bedrock_invocation_runs add constraint rg_bedrock_invocation_runs_provider_count_check check (provider_invocation_count between 0 and 1) not valid;
alter table public.rg_bedrock_invocation_runs validate constraint rg_bedrock_invocation_runs_provider_count_check;

create unique index if not exists rg_bedrock_invocation_runs_org_idempotency_uq on public.rg_bedrock_invocation_runs (org_id, idempotency_key);
create index if not exists rg_bedrock_invocation_runs_org_env_created_idx on public.rg_bedrock_invocation_runs (org_id, environment_id, created_at desc);
create index if not exists rg_bedrock_invocation_runs_batch_idx on public.rg_bedrock_invocation_runs (org_id, batch_id, batch_index);
create index if not exists rg_bedrock_invocation_runs_proposal_idx on public.rg_bedrock_invocation_runs (proposal_id) where proposal_id is not null;
...
create unique index if not exists rg_bedrock_invocation_locks_run_uq on public.rg_bedrock_invocation_locks (run_id);
create unique index if not exists rg_bedrock_invocation_locks_idempotency_uq on public.rg_bedrock_invocation_locks (org_id, idempotency_key);
create index if not exists rg_bedrock_invocation_locks_org_env_idx on public.rg_bedrock_invocation_locks (org_id, environment_id, acquired_at desc);
...
comment on table public.rg_bedrock_invocation_runs is 'Durable organisation/environment-scoped state for governed Amazon Bedrock invocation console runs.';
comment on table public.rg_bedrock_invocation_locks is 'Database-enforced at-most-once execution locks for governed Bedrock invocation runs.';
