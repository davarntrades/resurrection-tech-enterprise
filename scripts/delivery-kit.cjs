...
table{width:100%;border-collapse:collapse;margin-top:8px;font-size:11px}table.creg{table-layout:fixed;font-size:10px}table.creg th,table.creg td{padding:6px 7px}table.creg td{overflow-wrap:anywhere;word-break:break-word}table.creg th{overflow-wrap:normal;word-break:normal}th{text-align:left;font-size:9px;letter-spacing:.08em;text-transform:uppercase;color:#6b7480;padding:7px 9px;border-bottom:1px solid rgba(255,255,255,.1)}td{padding:8px 9px;border-bottom:1px solid rgba(255,255,255,.06);color:#cdd6e0;vertical-align:top}td.m{color:#f3f5f7}td.n{font-family:ui-monospace,Menlo,monospace;color:#e0a93f;text-align:right}
...
table{width:100%;border-collapse:collapse;margin-top:8px;font-size:9pt}/* Connector evidence register: 11 columns on a 6.5in page. Auto layout sizes    columns to the longest token, so one model ID (anthropic.claude-sonnet-4-5-   20250929-v1:0) widened the table past the page and CLIPPED the response hash    and hash-check columns out of the PDF entirely — present in the HTML, absent    from the artefact an auditor reads. Fixed layout + explicit widths + wrapping    keeps every column on the page. */table.creg{table-layout:fixed;font-size:7.5pt}table.creg th,table.creg td{padding:5px 6px}table.creg td{overflow-wrap:anywhere;word-break:break-word}table.creg th{font-size:6.5pt;letter-spacing:.02em;overflow-wrap:normal;word-break:normal;hyphens:none}th{text-align:left;font-size:7.5pt;letter-spacing:.06em;text-transform:uppercase;color:#212121;font-weight:700;padding:6px 8px;border-bottom:1pt solid #212121}td{padding:6px 8px;border-bottom:0.5pt solid #d9d9d9;color:#333;vertical-align:top}td.m{color:#212121}td.n{text-align:right;color:#212121}
...
function connectorEvidenceHtml(ce) {
  ...
  return `${head}
    ...
    <p style="margin:0 0 12px;color:#737373;font-size:12px">Lifecycle status is the proposal's terminal state — it is not the Ω verdict. A record reading <em>executed</em> reached its provider after a permitted decision; the verdict itself, and every refusal, is in the governance decision log.</p>
    ...
    ${reg.length ? `<h3 style="margin-top:18px">Evidence register</h3><table class="creg"><colgroup><col style="width:11%"><col style="width:10%"><col style="width:10%"><col style="width:8%"><col style="width:8%"><col style="width:13%"><col style="width:5%"><col style="width:11%"><col style="width:8%"><col style="width:8%"><col style="width:8%"></colgroup><thead><tr><th>Evidence ID</th><th>Canonical action</th><th>Proposal</th><th>Lifecycle status</th><th>Outcome</th><th>Connector / provider / model</th><th>Calls</th><th>Gov / provider / total</th><th>Request hash</th><th>Response hash</th><th>Hash check</th></tr></thead><tbody>${reg.map((r) => `<tr><td class="m">${cell(r.evidence_id)}</td><td class="m">${cell(r.canonical_action_id)}</td><td class="m">${cell(r.proposal_id)}</td><td>${cell(r.governance_decision)}</td><td>${cell(r.execution_outcome)}</td><td>${cell(r.normalized_connector)} / ${cell(r.provider)} / ${cell(r.model)}</td><td>${r.provider_invocation_count}</td><td class="m">${esc(ms(r.governance_latency_ms))} / ${esc(ms(r.provider_latency_ms))} / ${esc(ms(r.total_latency_ms))}</td><td class="m">${esc(short(r.request_hash))}</td><td class="m">${esc(short(r.response_hash))}</td><td>${cell(r.evidence_hash_state)}</td></tr>`).join("")}</tbody></table>${ce.register_note ? `<p style="color:#737373">${esc(ce.register_note)}</p>` : ""}` : ""}
...
function connectorEvidenceMarkdown(ce) {
  ...
  L.push("_Lifecycle status is the proposal's terminal state — it is not the \u03A9 verdict. A record reading `executed` reached its provider after a permitted decision; the verdict itself, and every refusal, is in the governance decision log._", "");
  ...
  if (reg.length) {
    ...
    L.push("| Evidence ID | Canonical action | Proposal | Lifecycle status | Outcome | Connector / provider / model | Calls | Gov / provider / total | Request hash | Response hash | Hash check |");
    L.push("|---|---|---|---|---|---|---|---|---|---|---|");
    for (const r of reg) {
...
function safetyEnvelopeAuditHtml(governedResult) {
  ...
  return `<div class="sec page-break"><span class="eyebrow">Admissible Operating Envelope — bounded assurance</span><h2>${esc(safety.status || "UNAVAILABLE")}</h2>
